...
import { BoardUI, PIECE_SETS, DOOR_SETS, DEFAULT_PIECE_FIT, residueStep } from '../../play/js/board-ui.mjs';
...
function applyLook() {
  ...
  ui.setPieceFit({
    ...
    snap: params.has('piecesnap') ? params.get('piecesnap') !== '0' : options.pieceSnap ?? DEFAULT_PIECE_FIT.snap,
  });
}
...
async function resolveStage(L) {
  ...
  try {
    ...
    if (need > 0) {
      let found = null;
      if (L.autoCrop && (L.autoCrop.top ?? 0) + (L.autoCrop.bottom ?? 0) === need) found = cropStage(t, L.autoCrop.top ?? 0, L.autoCrop.bottom ?? 0);
