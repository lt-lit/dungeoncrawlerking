...
GLYPH on its wall (2026-09-18, the sledgehammer paragraph), and A CAST HINT MARKED BY ITS SPELL — the portal's ring, the ice's snowflake with the 3×3 patch framed (2026-09-21, THE SPELL GLYPHS paragraph).**
...
**THE SPELL GLYPHS ✅ 2026-09-21 (designer, on the ice build: "On move hints, there's just a square outline for both portal and ice. How am I supposed to know what spell it's suggesting?").** A cast hint was one frame for either spell. Now every PROPOSED cast — a hint, a cast in the analyzer's numbered PV lines, where the board does not show the spell yet — is marked BY ITS SPELL (`play/README.md` § "The spell glyphs"), and a PLAYED cast (the enemy's own under the red last-move mark, the analyzer's cast ply; arrows flagged `played`) keeps the bare frame, because the board shows the spell itself there and the first cut's glyph over a pair's rune ring hid the colour that says whose pair it is (replay-smoke's ring counts caught it): `pixelarrow.mjs PORTAL_GLYPH` a 10×10 ring, `ICE_GLYPH` a 9×9 snowflake, `drawSpell` stamping either in the arrow's colour with the arrows' black halo, centred on the square (the hammer's method — one bitmap for the board and the list); canvas-board's cast branch (an arrow's `cast` is the spell, `'ice'` / `'portal'`) frames a portal cast's square with the ring inside and, for the ice, frames THE 3×3 THE PATCH WOULD FREEZE (`#patchSquares`: the crop's squares around the centre less the terrain, the engine's `ice_patch`) with the snowflake on the centre, so the hint shows where the ice would lie; the hint list wears the glyph before the SAN (main.mjs `spellIcon`, `.hint-glyph`), and the enemy's cast and the analyzer (`lastMoveArrows` / `moveArrow` with `played`; `pvArrows` — a cast in a line numbered like the arrows, a pass drawing nothing) name the spell off the scroll letter. Gates: ui-smoke 383 ok / 0 failed (the ICE block's two glyph checks: an ice hint on s73 — the snowflake's centre and an arm in the rank-1 gold, the black halo, every floor square of b5's 3×3 framed and the wall c4 not, nothing framed outside, the list `1:ice 2:-` with its text unchanged — and a portal hint on the `?ice=off` page: the ring gold with its halo, hollow at its centre, the square framed alone, `1:portal 2:-`; hints off clearing both), replay-smoke 76 ok / 0 failed (its ring-pixel counts at plies 2, 3 and 5 of the portal sample are the played-cast guard — they failed on the first cut), selftest 51/51 headless; no Node module changed (pixelarrow.mjs is the board's and the page's alone), so the Node gates stand as recorded. Crops at a phone's dpr 3 (the board with an ice, a portal and a plain hint; the hint list) went to the designer.**
