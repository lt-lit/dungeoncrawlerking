...
- `play/js/deck.mjs` — THE DECK (2026-09-25; brief §4.10): spells as
  cards — the catalog (ice, portal, Reveal, Undo), the Adept starter, the
  enemy's deck by width, the seeded shuffle, THE POCKET AS THE HAND (a
  card's scrolls are the holdings; the meta cards beside), the draw to four
  at each turn's start (duel.mjs `#refill`, a bare position), the mulligan
  (a `--` ply), `deck` on every state and `metaPlays` on the record; the
...
- `engine/` — the Phase 1.2.3 patch kit: `patches/dead-squares.patch` (the
  ...
  **`play/vendor/` carries the PATCHED pair** (ice 2026-09-20 — the
...
node harness/test-deck-duel.mjs      # THE DECK in the duel (the vendored pair in Node — overlay first): the refill at each turn's start, the frozen pass and the link ply, the meta plays through an undo, the mulligan, no decks = the old controller
...
15. **The vendored pair is a MATCHED SET built from one patch, two trees,
    two toolchains — and EIGHT patches now: dead-squares, thread-stack,
    portals, wall-kinds, hammer, portals-body, portals-cast, ice, applied in
    that order (portals-body is portals-v2's body rule with the tunnel half
    removed and portals-cast is portals-v3 rebased, since the tunnel's
    retirement 2026-09-19; ice is the slide, the pit `_` and the ice scroll,
    2026-09-20).** ffish comes from FSF mainline (`src/Makefile_js`,
