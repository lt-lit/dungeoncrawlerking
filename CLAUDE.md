...
- `engine/` — the Phase 1.2.3 patch kit: `patches/dead-squares.patch` (the
  ...
  **`play/vendor/` carries the PATCHED pair** (dead-squares 2026-08-26 +
  ...
  2026-09-17 (one build, one gate) + portals-v2 2026-09-18, each behind a green rule-16
...
15. **The vendored pair is a MATCHED SET built from one patch, two trees,
    two toolchains — and SIX patches now: dead-squares, thread-stack,
    portals, wall-kinds, hammer, portals-v2, applied in that order.** ffish comes from FSF mainline (`src/Makefile_js`,
