...
- `play/js/rays.mjs` — PORTALS v2 ON THE GRID (2026-09-18): the one ray
  walker under the body-and-tunnel rule (`walkRay`; a grid carries its
  pairs as `grid.portals`, set by `fenGrid` / `gridOf`, kept by `copyGrid`)
  and `portalRoute` (which pairs a move's line ran through — the slide's
  legs, the arrows' `via`, the log). tactics.mjs and threat.mjs walk it;
  Node gate `phase0/harness/test-portals-game.mjs`.
...
- `engine/` — the Phase 1.2.3 patch kit: `patches/dead-squares.patch` (the
  ...
  **`play/vendor/` carries the PATCHED pair** (dead-squares 2026-08-26 +
  ...
  2026-09-17 (one build, one gate) + portals-v2 2026-09-18 + portals-v3
  2026-09-19 (the one-turn cast), each behind a green rule-16
...
node harness/test-portals-game.mjs   # PORTALS v2 on the grid (play/js/rays.mjs): the walker on the engine gate's fixtures, the route of a tunnel move, the gods' attacks / landing guard / pins / terrain reach / exposure rule through tunnels and stopped by bodies; Node only
FFISH_JS=$PWD/../play/vendor/ffish.js node ../engine/tests/test-portals-ffish.cjs      # THE PORTAL SPELL, ffish half (v2 + v3): the fixtures, SAN, the field, the strip rule, the check-flag sweep, the one-turn cast's frozen ply / link ply / fizzle
ENGINE_JS=$PWD/../play/vendor/stockfish.js node ../engine/tests/test-portals-engine.cjs  # THE PORTAL SPELL, engine half (v2 + v3): perft on the fixtures over UCI (pinned to the native build), the casts through `d`, the strip at the root, the search, the one-turn cast
cd ../engine/forge && python3 native-test.py <native-largeboard-build> --fixtures --random 800 --sweep --depth 4  # THE FORGE's native gate for the portal patches: hand-derived fixtures, the independent Python oracle (oracle.py) on random positions with pairs, scrolls, halves and passes, the scratch tree's xsweep, perft under a debug build's asserts
...
15. **The vendored pair is a MATCHED SET built from one patch, two trees,
    ...
    portals, wall-kinds, hammer, portals-v2, portals-v3, applied in that order.** ffish comes from FSF mainline (`src/Makefile_js`,
