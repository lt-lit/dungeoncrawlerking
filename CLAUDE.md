...
displacement its blue arrow alone, a hammer arrow ending in THE HAMMER
GLYPH on its wall (2026-09-18, the sledgehammer paragraph), and A CAST HINT MARKED BY ITS SPELL — the square framed at its edge with the portal's ring or the ice's snowflake inside, a drop shadow and no halo (2026-09-21, THE SPELL GLYPHS paragraph).**
...
PORTALS NEVER ON A KING ROW (the promotion zones) — and since 2026-09-20
NEVER ON THE ROW BESIDE ONE either ("PORTALS v4.1" below; the cast's
region is ranks 3…R−2) — so no pawn promotes through a portal, the engine
carries no promotion branch for the move, and the easy queen costs two
moves of walking. Spells are NEVER pieces: a king
...
the arrow colour (BY SPELL since 2026-09-21 — THE SPELL GLYPHS paragraph); the log says what a cast did and where a portal move
...
DESIGNER'S ORDER: THE SLEDGEHAMMER (✅ built the same day — the next
paragraph), then an ICE spell. Held
...
**THE SLEDGEHAMMER AND THE INDESTRUCTIBLE OBSTACLE `#` ✅ BUILT 2026-09-17
(designer, after the discussion below: "Go ahead and build it. Let's make
all kings into sledge-kings for testing"; the discussion's rulings — "A
cracked wall is a cracked wall, who cares who made it?", and on the hammer
as a free tempo "Who cares, there's a finite number of walls to crack, it's
not an infinite stall", "Agreed with the rest": SAN `K*d1`, one bedrock
look for every theme, one forge session for both patches, the meter blind
to a hammer, only the player hammers on the walk; `#` KEPT — "we do not
HAVE to use the # symbol" was raised and answered: the mate suffix and the
board glyph never share a string, no URL parameter carries a FEN, the ini
parser reads `#` as a comment only at a line's start, a shell only at a
word's, and every alternative collides worse).** THE DESIGNER'S FOUR RULES
stand as decided (brief §4.8): a piece with the sledgehammer SPENDS ITS
MOVE turning an ADJACENT breakable wall into `^`; a property of a PIECE, not
a spell; designed for THE KINGS; a second kind of wall IN THE ENGINE — `*`
the BREAKABLE wall, `#` ANY INDESTRUCTIBLE OBSTACLE (a pit, the map's edge,
bedrock, one day a moving wall), ONE THING to the engine, told apart for
the eye by the game's own ledger. TWO ENGINE PATCHES, one forge session,
one rule-16 gate (`engine/README.md` § "The wall-kinds patch and the
hammer patch"; `engine/patches/wall-kinds.patch` 131 lines, `hammer.patch`
265, applied after portals; the toolchains reinstalled from scratch — the
container keeps nothing): `#` parsed into `wallSquares` with a
`hardSquares` subset, `Zobrist::hard` from its own PRNG (the stock key
sequence untouched), emitted and validated as `#`; a HAMMER move type (9
in the 4-bit field, after PORTAL) from the piece to the adjacent breakable
wall (the eight king steps), plain `e1d1` notation — EVERY MOVE ONTO A `*`
IS A HAMMER, the portal trick again — SAN `K*d1` (the piece, the wall
glyph, the square; never `x`, it is not a capture), generated in QUIETS
and NON_EVASIONS only, legal exactly when not in check (nothing moves and
the square stays blocked, so a hammer neither gives nor resolves check —
three explicit branches the discussion found: `gives_check` false, `legal`
= `!checkers()` with the `board_bb() & to` assert relaxed since `board_bb`
excludes wall squares, `pseudo_legal` by regeneration since a TT hammer's
destination is a wall), do = the wall bit off, the dead bit on, the two
keys swapped, rule50 reset, no piece moves; undo = the state pointer (the
wall and dead occupancy XORs already in `undo_move` cancel on the hammered
square, the piece move skipped as a drop's is); SEE zero by type;
`key_after` the same swap; the variant key `hammerPieceTypes` (+ `White` /
`Black`, a PieceSet per colour — so "this king hammers, that one does
not" is a deal-variant setting and costs no new letter), NO_PIECE_SET
stock-identical. THE GAME, THE WALL KINDS: `fen.mjs` `HARD` + `isTerrain`
(the 1.2.4 landmine again — an unknown glyph read as a white piece; the
helper at some 35 files was the fix, the explicit wall tests asked one by
one "obstacle or breakable?") + `isWall`; `world.mjs` BEDROCK, a fifth
terrain kind — the world file's `#`; a hole or bedrock is `#` to the
engine, a wall `*`, every off-map square `#`; the ledger decides what a
`#` (or an old log's `*`) is, so both committed samples load unchanged —
the run stamp `dck-run/5`; `stage.mjs` drops the alias (`*` wall, `#`
bedrock, nothing else); the 36 arenas + the archive rewritten `#` → `*`
(573 + 392 cells, the map rows alone, formatting untouched), the four
fixtures regenerated (`dungeon.mjs` exports the ring `#`, every other
stone `*`; the working floor knows one stone); the Director's crumble
writes `#` (`crumbleFilter collapseFen`), the census reports `hard`;
`classifyCell` gains `bedrock` (a `#` outside the holes ledger: a standing
wall to every mask, painted by canvas-board's `#bedrockTile` — the theme's
wall case pulled toward grey and darkened, `BEDROCK_GREY` 0.45 /
`BEDROCK_SHADE` 0.6, a paint-time composite that follows every theme and
tone, no atlas row); the report prints the FEN's glyphs (`*` / `#`, `O`
the ledger's hole); the meter reads a hammer as cold (a crate smash already
got zero credit, which settled the old furniture-farming worry by
omission — the hammer is a pass, and a cold ply is its price; the stock
pair now fails LOUDLY on a `#` board, so a phase0 run that forgot the
overlay dies at once). THE HAMMER IN PLAY: `variant.mjs hammerIniKeys`
(`hammerPieceTypes = k`, `HAMMER_PIECES`), `dealVariant(..., { hammer })`
under the suffix `__sledge` (rule 7), threaded beside `portals` through
`dealMatchup` / `planBox` / `planBarrier`; `?hammer=off` and Options →
Spells "Sledge-kings" (`options.hammer`, on by default); `duel.mjs
hammerOf` (the move's destination was a `*` before it) marks
`lastMove.hammer` — every recorded state carries it — and adds the square
to the Director's god-crate set (ONE LEDGER, the designer's: a cracked wall
is a cracked wall, whoever cracked it — the art, the crate brake and the
breach rung see one kind, and the move list still says who); main.mjs
`onMove` slides nothing on a hammer — the wall wears the gods' weaken beat
and drops its chips (a `weaken` debris event), the log reads "K*d2 — the
sledgehammer cracks the wall at d2"; the tap path needed nothing new
(ffish lists `e1d2`, `targetsFor` lights the wall, the target mark paints
over the tall pass); THE WALK (ruling 11): `army.hammer` (set from the
option on every army the walk builds or restores, saved with the army),
`pieceMoves` offers the adjacent `*` cells as `'hammer'` captures of the
king alone, `planTurn` returns `plan.hammer` with no moves (the box is
what it was), `applyTurn` writes `^` and the world's cracked-wall ledger,
the enemies get their turn, the crate is opened by whoever captures it
(a hunter's path still treats it as a wall — automatic moves never take
furniture), the debris a `weaken` by cell, the status "cracks the wall";
the save records the input as a move. GATES (the new pair vendored, both binaries): NATIVE FIRST (`forge/native-test.py`, a debug largeboard build with the asserts on and `pos_is_ok` after every undo, over UCI: 28 checks — the king beside a `*` and a `#`, in check no hammer, a PINNED rook hammers, `#` ≡ `*` without the key at perft 3, per-colour keys, a 10×10 duel-shaped search, perft 3–4 on every fixture), then test-hammer-ffish 27 (the same through the JS API — SAN, push/pop, perft 1–4 = 5/30/182/1338 · 7/59/508/6521 · 6/36/241/1853, a 1,505-move check-flag sweep with 0 mismatches), test-hammer-engine 25, regress + regress-ffish (crate-free boards node-identical to the shipped pair), test-engine 7, test-ffish 19, xcheck, search-identity node-for-node at d12, stack-regress 5, test-portals-ffish 42, test-portals-engine 55, depthcap d22 110/110 (slowest 1613 ms) and d60 30/30 — the cap STAYS at d22; selftest 48/48 headless (the sledgehammer check: K*f2 on the game's own deal variant, the hard f1 never, none in check, the engine's perft 5 and a legal bestmove, plain kings without a hammer, `#` ≡ `*`); test-world 130, test-dungeon 96, test-barrier 162 (a sledge deal's name and key), test-army 131 (the walk's hammer: offered, planned as a move of nobody, the ledger, the crate a capture next, bedrock never, the save), test-enemy 100, test-camera 80, test-debris 76, test-logreport 53, test-armygen; ui-smoke 336 ok (THE SLEDGEHAMMER block: on s65-guard-post seed 1 the kit's king deals at e1 with walls at d1 and d2 — a tap on him lights both beside his moves, a tap on d2 cracks it: the state after the ply a crate in the ledger and marked `hammer`, SAN K*d2, the log line, the king still on e1, the board painting it cracked; `?hammer=off` lights no wall; on the walk the king's manual hammer plans as a move of nobody, the world's ledger takes the cell, the crate is a capture next, the save carries it), replay-smoke 69 (both samples unchanged: an old log's `*` holes read through the ledger), facing-walk 108/108, canvas-grid `none` / `margin` 4/4 in Chromium; the world-shots gallery re-rendered (the bedrock ring around every floor). THE PHONE VERDICT CAME THE SAME DAY (designer, 2026-09-17, with the log): "Seems to work pretty good on mobile. I did a few sledge king moves but portals are naturally a lot more powerful. About what I expected. I definitely saw the engine go for at least one sledge move, so it definitely evaluates the option." — IN. THE LOG IS THE THIRD COMMITTED SAMPLE, THE FIRST SLEDGE DUEL (`replay/samples/dck-log_vaults-4-t109_s3010228489.json`, `?sample=3`: vaults-4 at walk turn 109, the enemy's initiative, wrathful, Android Firefox, 148 plies, 25 quakes of 147 rolls, 0 rejected draws, 0 undos, 0 anomalies, 1-0 by extinction at ply 148). WHAT THE RECORD SAYS: ONE hammer was played, the player's K*e4 at ply 16 — the very reply the enemy's depth-22 search had predicted (`predicted d3e4`, `followed`), its line running on with fxe4 taking the crate and a second K*e3; at ply 115 its line carried K*f6 and K*f7 — so the engine prices the hammer in its own lines (the designer's "evaluates the option"), though the enemy king never hammered in this duel; the state after ply 16 carries `hammer: e4` with e4 in the gods' crate ledger from that ply on and `^` in the FEN; every crumble was written as `#` and the holes ledger names all twenty of them at the end (none at the start), so the analyzer paints them as pits and never as bedrock. test-logreport 61 and replay-smoke 74 ok read the sample (the hammer's state and SAN, the crate from that ply, the pits through the ledger). THE HAMMER GLYPH ✅ 2026-09-18 (designer, on the verdict: "Let's go ahead and get a hammer glyph hint. No to enemies using it while walking, that's a ridiculous idea"): an arrow onto a breakable wall — a hint, the enemy's own hammer (the red last-move arrow), the analyzer's ply arrow and its numbered PV arrows — ends in A SLEDGEHAMMER stamped upright on the wall's square (`pixelarrow.mjs HAMMER_GLYPH`: an 8×4 head in the arrow's colour over a 2×6 handle in that colour's shade — `shadeHex`, `HAMMER_SHADE` 0.55 — with the arrows' one-pixel black halo, rows 3–12 and columns 4–11 of the square, on the wall's face; `drawArrow`'s `hammer` option paints it after the arrow on the same scratch, so the head under it never double-blends; UPRIGHT at every angle — a sprite turned to a diagonal is mush at this size, and the arrow already says where it came from), and THE HINT LIST wears the same drawing (main.mjs `hammerIcon`: a canvas per hammer hint in the rank's arrow colour between the rank and the SAN, so the line's text reads as ever — "1 K*d2 +0.4"). WHO KNOWS A MOVE IS A HAMMER: `fen.mjs hammerOf` (moved from duel.mjs, which re-exports it — the destination was a `*` before the move), read by `applyHintLines` on the live board, by `lastMoveArrow` from the record's `hammer`, by the analyzer's `moveArrow` from the state's `hammer` and by its `pvArrows` walking a line's walls (a later move onto a cracked square takes the crate — no glyph); canvas-board passes the flag through; `__DCK.paintHints(pvs, n)` is the smoke's hook into the probe's own paint path. ENEMIES NEVER HAMMER ON THE WALK — ruled out, not held over ("a ridiculous idea"; a hunter's path treats a wall as a wall, as built). Gates: ui-smoke 336 ok (THE SLEDGEHAMMER block grew the glyph: three lines injected through the probe's path with the arrows at full opacity — the list's text unchanged with the icon on the two hammers alone, the board's arrows flagged worst to best, d2's head gold / handle its shade / halo black to the pixel, d1's rank-2 hammer in its own colour at its strength's opacity, the plain hint's square without one pixel of the handle's shade, hints off clearing them), replay-smoke 76 ok (sample 3 at ply 16: the ply's arrow flagged, e4 wearing the gold hammer over the cracked wall), selftest 48/48, test-logreport 61, the other Node gates unchanged; crops at k 6 (the phone: the board, the hint list, the analyzer's e4) went to the designer. Held over: the sledge-king sprite (both kings look as they do; the tell is the lit walls when you tap him, and now the hammer on the hint), the hammer as an upgrade (per colour is a deal-variant setting; a non-royal hammer piece would be a custom type), an ICE spell next.**

**PORTALS v2 — THE BODY AND THE TUNNEL ✅ BUILT 2026-09-18 (designer, after the sledgehammer: "1. Is it possible to make it so portals block sliders? 2. Is it possible to make it so sliders can go fully thru both portals without stopping at all? Like you'd click on your bishop and see it's available landing squares go into one portal and out of the other?" — one discussion, then "go ahead and build it. Portals v2"). THE RULES (brief §4.7 "Portals v2"): a linked portal square is a BODY — every line stops at it, whatever stands on it (a half is not a body until linked); an EMPTY PAIR is a TUNNEL for sliders — a rook's, bishop's or queen's line entering an empty portal whose twin is also empty comes out of the twin in the same direction and runs on, through another empty pair too, each pair once per line; stopping at the entry is the ordinary portal move (the piece ends on the twin or swaps, never on the entry); anything on either square, either colour, closes the tunnel (a landing still swaps through a plugged pair); a pawn's double step is two steps, not a slide (a portal on the first square ends it, a portal on the second is landed on). REJECTED BY THE DESIGNER on the way: capture at the exit ("capturing two pieces is a no no… this is shaping up to be a lot of special cases") — landing swaps regardless, as v1; a tunnel for the double step (it would promote through a pair and break en passant). What follows and is built as real attacks: check through a tunnel, a pin through it, a discovered check off a portal square, stepping into an open pair as a block, a linking cast as an interposition (never in check), a linking cast that GIVES CHECK through the new pair, a self-exposing linking cast ILLEGAL. THE ENGINE HALF is `engine/patches/portals-v2.patch` (650 lines; position.h, position.cpp, movegen.cpp; applied after hammer — SIX patches now; `engine/README.md` § "The portals-v2 patch"): every ray passes its occupancy through the bodies (`attacks_from` / `moves_from`, `attackers_to`, the check squares, `slider_blockers`' lines — an empty portal square on a line is never a blocker of record — SEE's x-rays, the promotion checks), `portal_through` walks a rider's lines on through open pairs (magic rays to the entry, a stepped walk from the twin, chained once per pair, never a wall, never the origin), `tunnel_attackers` joins them to `attackers_to` — so FSF's `legal()`, which already judges every move on a virtual occupancy, reads the new rule for free; `portal_attacks_king` keeps the relocations of a PORTAL move and now judges a LINKING CAST with the new pair in the table (legal: the caster's king; gives_check: the enemy's); `gives_check` adds the mover's tunnel line from its new square and the discovered attacks on the new occupancy while a pair is open; movegen adds the tunnel squares to a rider's attacks and quiets (the bitboards dedupe a square reached plainly and through a pair), the pawn's double step never crosses a portal, and IN CHECK WITH AN OPEN PAIR EVERYTHING IS GENERATED and `legal()` decides (a leaper's check too, a double check too — a landing may swap the king out of either); `pseudo_legal` accepts a rider's tunnel move and validates evasions by regeneration. Stock when no pair stands: portal-free boards node-identical (perft 4 and depth-12 transcripts) to the five-patch build. NATIVE FIRST, an INDEPENDENT ORACLE THIS TIME (`engine/forge/oracle.py` — committed, with `native-test.py` the UCI driver and `portals-v2.ini` the test variants: the rules as Python from the rule text, forward rays, make/unmake by copy): 57 hand-derived fixture checks (the oracle confirmed every hand count before the engine ran them — and corrected one of mine: Kc3 in the old F3 is refused because the enemy rook reaches c3 THROUGH the pair), the engine's own consistency sweep (`xsweep`, a scratch UCI command: pseudo_legal / legal vs the king's real attackers after the move / gives_check vs the real check / the incremental state vs a fresh position / the undo, over every generated move to depth 3), and 1,500 random positions on 6×6, 8×8 and 10×10 boards with walls, crates and one to five pairs — perft-1 move sets and perft-2 per-move counts equal to the oracle's, the sweep clean — which caught TWO real bugs before any WASM was built: a captured piece's own tunnel line looping back to its square counted it as an attacker of the square it was taken on (a legal king capture refused — a line's origin is excluded from its reach now), and the evasion fallback's leaper narrowing dropped the portal landings that swap a king out of a knight's check. Debug build perft 5 on twelve fixtures under `givesCheck == checkers` and `pos_is_ok`. THE GATE, both binaries rebuilt from the clean pinned trees + six patches (js and worker byte-identical in size to the vendored, wasms +11 KB / +10 KB): test-portals-ffish 64/64 (rewritten for v2, a 2-ply check-flag sweep over twelve fixtures), test-portals-engine 129/129 (perft 1 sets and perft 3 totals pinned to the native build, the boards after the key moves, the queen taken through a tunnel by search, a 10×10 duel-shaped search alive), regress + regress-ffish identical, test-engine 7, test-ffish 19, xcheck, test-hammer-ffish 27, test-hammer-engine 25, stack-regress 5, search-identity node-for-node at d12, depthcap d22 110/110 (slowest 1335 ms) and d60 30/30 — THE CAP STAYS AT d22; selftest 49/49 headless (a v2 check on the game's deal variant: the fourteen moves with Rh8+ through a4–h5, the landing, the plugged exit's swap, the double step ending at the portal, engine perft 14 and a1g7 through the tunnel, the grid walker agreeing). THE GAME HALF: `play/js/rays.mjs` — ONE ray walker (`walkRay`) under the v2 rules for every grid consumer (a grid carries its pairs as a non-enumerable `portals` property set by `fenGrid` / `gridOf`, kept by `copyGrid`), and `portalRoute(fenBefore, uci)` — which pairs a rider's line ran through (`pairs`, `path` = [from, entry, exit, …, to]; a plain landing is plain; a square reachable plainly reads plain); tactics.mjs (`attacksFrom`, the pins and skewers with the walked squares as the line, `terrainReach`) and threat.mjs (`buildChains` x-raying on from each occupant, `editExposes`) walk it, so the gods' landing guard and exposure rule see through tunnels and stop at bodies; canvas-board `animateSlide` takes a `path` and slides the legs with a CUT at each pair (the eased progress spread by length), `#paintArrows` draws an arrow with `via` in pieces — into the entry, out of the exit, on to the destination, the label and the hammer on the last; main.mjs reads the route in `onMove` (the slide's legs, the traffic per leg, the log "— via c3→f6[, through the portal to …]"), `applyHintLines` (the arrow's `via`, the list's "via c3→f6" in a `.hint-via`), `lastMoveArrow` (off `fenBeforeLast`), and THE EXIT ALIAS: an empty twin of a lit landing lights too (`exitAliases`) and a tap on it plays the landing (the picture the designer asked for); replay.mjs's `moveArrow(st, prev)` and `pvArrows` carry `via` (the PV's routes read off the line's start position). Gates: `test-portals-game.mjs` 34 (the walker on every fixture, the route, attacks / landing safety / the pin through the tunnel / the terrain reach / the exposure rule with and without the pair, copies keeping pairs, and THE LOOP — a line that comes round through two pairs to its own square ENDS THERE, unvisited, the engine's `& ~square_bb(s)` on the grid: the smoke's random driver found the landing guard's x-ray chain looping forever on one, so `walkRay` never visits its origin and `buildChains` keeps a seen set), selftest 49/49, ui-smoke 366 ok (a PORTALS v2 block: an arrow `via` painted in two pieces and nothing on the cut, a slide in legs resolving clean, the exit alias on the live duel when a landing on the player's own pair comes up), replay-smoke 76, test-logreport 61, test-debris 76, test-camera 80, test-world 130, test-barrier 162, test-army 131, test-enemy 100, test-dungeon 96, test-armygen, phase0's `lib/selftest.mjs` under the overlay. THE PHONE VERDICT CAME THE NEXT DAY (designer, 2026-09-19): "Seems to work fine on the phone" — Portals v2 is IN; the branch merges as it stands. Held over: the eval's mobility through tunnels (the search sees the moves, the eval counts plain lines), SEE of a portal move (still the stock zero), the analyzer's PV routes read off the line's start rather than each ply's board, a flash of the rings as a piece passes, the atlas sprite.**

**PORTALS v3 — THE ONE-TURN CAST ✅ BUILT 2026-09-19 (designer, on the v2 build: "Is it possible to make it so both portals are placed in one turn instead of two?" — the answer on record: a free pair as ONE engine move is quadratic, about 1,000–2,400 legal casts at every node on a 10×10 while the scrolls are in hand (the move array holds them, `MAX_MOVES` 8,192 in this build; the search does not — its depth about halved, every node spent listing casts), so the recommendation was THREE PLIES the other side cannot use; the designer: "It might make portals too strong but let's go ahead and try it"). THE RULES (brief §4.7 "Portals v3"): an open half FREEZES the other side — its only legal move is a PASS (its king's square twice, `e8e8`; SAN `--`), played by the game itself, no search and no tap; an open half BINDS its caster to the link — its only legal moves are the linking casts (v2's self-exposure rule still refuses the twins that would open an enemy slider onto the caster's king); a half NO LINK CAN CLOSE FIZZLES — the caster's only move is a pass that removes the half (its scroll spent, the other kept; a geometry that never happens on a real board, it exists so the rule never leaves a side moveless); a pass is nothing (no check, no capture, and two passes in a row — the frozen pass, then a fizzle — end nothing: FSF's double-pass draw is for other variants' passes); nobody is ever in check while a half is open (a half is cast only out of check and moves nothing, the frozen side cannot move), so the evasions never see the freeze — a hand-written position in check plays its ordinary evasions. To the gods the three plies are ONE action: the meters, the repetition record and the quake roll skip the half and the frozen pass and see the link (or the fizzle) once. Considered and not built: a fixed twin derived from the entry (one move, no cost, a different spell); the half not ending the turn (a true double move — surgery in a search that assumes alternation everywhere; every patch stays on the rules side). THE ENGINE HALF is `engine/patches/portals-v3.patch` (185 lines; position.h, position.cpp, movegen.cpp, apiutil.h; applied after portals-v2 — SEVEN patches now; `engine/README.md` § "The portals-v3 patch"): the state is the FEN's (`half_open()` / `in_cast()` read the portals patch's `portalHalf[c]`; a half never outlives the link ply, so "the opponent's half is open" IS "frozen" — no flag, no grammar change, a bare `position fen` reproduces the move set), movegen's early return under `in_cast()` (the scroll's `generate_drops` for the caster plus FSF's own pass `make<SPECIAL>(ksq, ksq)`; the pass alone for the frozen side; nothing for CAPTURES; never for EVASIONS), `legal()` (the frozen pass legal; the caster's pass legal only when no cast in `MoveList<QUIETS>` is), `pseudo_legal()` by regeneration inside a cast, `gives_check()` false for every pass (stock would have read a king between its own rook and the enemy king as discovering a check by standing still — latent for FSF's pass variants), `do_move()` setting `st->pass` for a VARIANT's pass only and XORing the fizzled half's key out, `key_after()` for a pass, SAN `--`. CLOSED ON THE WAY, a gap in portals.patch: a NORMAL-typed move onto a portal square was pseudo-legal (movegen encodes every such move as PORTAL), so a colliding transposition-table move could be tried as a landing that does not teleport — the debug build's MovePicker assert found it during the one-turn cast's search tests (`4k3/3p4/8/8/8/8/3P4/4K3[oo] b - - 0 2 {c4-e7}`, `e8e7` typed NORMAL); now not pseudo-legal. NATIVE FIRST: the oracle (`engine/forge/oracle.py`) grew scrolls, halves, the opening cast, the link with its exposure test, the frozen pass, the fizzle and `from_fen`; every V3 fixture count confirmed by it before the engine ran; 64/64 fixtures on the debug build (asserts on) and the release; 1,800 random positions (scrolls 0–2 a side, a half open for one side on a third of them, 1–3 pairs, walls and crates, 6×6 / 8×8 / 10×10) equal to the oracle on perft-1 move sets and perft-2 per-move counts, 1,400 of them under `xsweep` (taught the fizzle rule); perft 4 on 15 fixtures under asserts; portal-free boards node-identical to the six-patch build (perft 3 = 61,433, depth-12 transcripts identical). THE GATE, both binaries rebuilt from the clean pinned trees (the glue byte-identical; the wasms +3,402 / +1,457 bytes): test-portals-ffish 77/77, test-portals-engine 141/141 (perft 3 re-pinned to the native build where scrolls are in hand: F10a 16,214, F10b 33,603, F16 2,070, F17 4,248, the 10×10 duel shape perft 2 = 1,893), test-hammer-ffish 27, test-hammer-engine 25, test-ffish 19, test-engine 7, regress + regress-ffish identical, xcheck, stack-regress 5, search-identity node-for-node at d12, depthcap d22 110/110 (slowest 1,551 ms) and d60 30/30 — THE CAP STAYS AT d22. THE GAME HALF (`play/README.md` § "Portals v3"): duel.mjs `forcedMove()` / `mustLink()` / `playForced(mover)` and `castKind` (every recorded state carries `cast`: half / link / pass / fizzle) with the gods skipping the half and the pass plies; main.mjs `driveTurn` plays a forced pass for whoever is to move (the enemy's at once, the player's after a 600 ms beat so the enemy's half is seen), puts the player INTO cast mode on the link ply ("place the second portal"; `setCastMode` holds it, a stray tap is nothing), the log's words for a frozen side and a fizzle, no slide or arrow for a pass; the analyzer draws no arrow for a pass; `fen.mjs isPass`. Gates: selftest 50/50 (the v3 check; the v1 check's four casts are six plies now), ui-smoke 326 ok (THE PORTAL SPELL block reads the one-turn cast: the enemy's pass in the record after the first tap, the player on the link ply in cast mode with no piece move offered, the status and the log, the kinds half / pass / link, the gods rolling on the link ply alone), replay-smoke 76, test-logreport 61, the Node gates unchanged. Held over: the enemy's link at a shorter `go` (its cast is two searches around the player's automatic pass), the two-tap gesture as one drag, the atlas sprite; the designer's own note going in — "It might make portals too strong" — is the question the phone answers.**

**PORTALS v4 — THE TUNNEL RETIRED ✅ BUILT 2026-09-19 (designer, on the s88 log — "Portals lead to some pretty wacky situations. Thoughts?" — after the assessment: "I'm actually considering getting rid of the true pass thru moves for sliders, and making it so entering a portal always stops movement and lands on the exit portal. It's really cool that the pass thru moves actually works, but I'm worried it's not intuitive to read." — then "I like the swaps, at least they're consistent. Let's make v3 portals without the pass thru moves. We'll keep the double portal cast and the portals stopping movement. Everything going thru a portal simply lands on the exit portal now, swapping if there's something there. Nice and consistent and readable. We might bring back the pass thru portals later… Perhaps it'll be a late game upgrade.").** THE RULE IS ONE SENTENCE: a piece that moves onto a portal comes out of the other one, and if something stands there they swap (brief §4.7 "Portals v4"). THE BODY RULE STAYS (a linked portal square ends every line, whatever stands on it; a half is not a body until linked; a pawn's double step never crosses one); THE TUNNEL IS GONE — a rider's line that reaches a portal square ends there and the move onto it is the landing; with it went every special case it carried: check, pins and discovered checks through a pair, a linking cast that gives check, the self-exposure filter on the link (bodies only close lines, so EVERY castable square is a legal link and the fizzle is unreachable on a real board — it stays as the safety net), the loop of a line coming round through two pairs. THE SWAP, the capture-then-teleport, kings through portals and the one-turn cast stay. WHY: legibility — the tunnel's moves were readable on screen (the arrow in legs) but its THREATS were not (on the s88 log the enemy's rook on b9 struck d3 through the player's own pair; a check or a pin through a pair has nothing on the line to read); on that log the tunnel made 3 of the 15 portal plies, and the twelve teleports (four swaps, three king moves, the two-move queen off a rank-8 portal) are untouched by this change and stay on the table. THE PATCH SET is still seven, named by what they do now: `engine/patches/portals-body.patch` (portals-v2.patch with its tunnel half removed — 61+/26− against v2's 309+/55−: `attackers_to` adds the bodies in one line at the top, the evasions are v1's again, `portal_attacks_king` is v1's shape judged through the bodies) and `portals-cast.patch` (portals-v3.patch rebased; `engine/README.md` § "The portals-body patch" / "The portals-cast patch"). NATIVE FIRST: the oracle (`engine/forge/oracle.py`) lost its pass-through and the link's exposure test, and re-derived every fixture count before the engine ran (it corrected one of mine: h6g7 is the bishop's own direct check, so F5 reads "only h6g7"); 62/62 fixtures on the debug build with asserts on and on the release (F1 11 moves and no check from the landing, F4 not in check with 13 free moves, F6 not in check, F7/F8 ten moves — the first pair the body, no chain — F10a's 45 links none checking, F10b all 46 legal, F14 22 with d2c3 legal, the shield B1, the fizzle on a NEW board with one castable square in all — v3's fizzle board links freely now); 1,800 random positions vs the oracle (800 debug + xsweep, 600 release with more pairs + xsweep, 400 release) with 0 mismatches; xsweep 3 clean; perft 4 on 16 fixtures under asserts; portal-free identity vs the tunnel build — perft 3 61,433 and depth-12 transcripts identical on four boards with NO PAIR AND NO SCROLL (a board with scrolls in hand is not portal-free to a search: it casts pairs deep in the tree, where the rules differ — the first identity run's "difference" was that, not the engine). THE GATE, both binaries rebuilt (js and worker byte-identical to the vendored, the wasms −10,849 / −8,745 bytes): test-portals-ffish 78/78 (rewritten), test-portals-engine 146/146 (perft 3 re-pinned to the native build; the queen on its portal square taken by the rook or the pawn — the engine now prefers d2xc3, both land the captor on f6), test-hammer-ffish 27, test-hammer-engine 25, test-ffish 19, test-engine 7, regress + regress-ffish identical, xcheck, stack-regress 5, search-identity node-for-node at d12, depthcap d22 110/110 (slowest 1,410 ms) and d60 30/30 — THE CAP STAYS AT d22. THE GAME HALF is mostly deletion (`play/README.md` § "Portals v4"): `rays.mjs walkRay` under the body rule (a portal square is visited and ends the walk; no `through`, no origin guard), `portalRoute` gone with every `via` — canvas-board's slide legs and arrow pieces, main.mjs's route in `onMove` / `applyHintLines` / `lastMoveArrow` (`fenBeforeLast` deleted), the hint list's "via" and its CSS, replay.mjs's `moveArrow(st)` / `pvArrows`; threat.mjs's `buildChains` lost its seen set; THE EXIT ALIAS STAYS (it is the picture the designer wanted). Gates: test-portals-game 27 (rewritten), selftest 50/50 headless (the v4 check), ui-smoke 373 ok (the portal block reads a landing's picture; one run died on the engine transport glue on record — `bestmove … ponder …readyok` as one line — the rerun green, engine.mjs untouched), replay-smoke 76 ok, test-logreport 61, the other Node gates unchanged (test-debris 76, test-camera 80, test-world 130, test-barrier 162, test-army 131, test-enemy 100, test-dungeon 96, test-armygen, strip-ruin-chips), ladder-smoke 12/12 terminated (10.4 quakes a game, double-touch 0, 3% on the very next ply, none in check). Held over: the tunnel as a late-game upgrade (its engine, oracle and grid halves are in the history at the v2 and v3 commits 59eaa82 / 41e49e1; an upgrade would be a per-deal or per-side variant setting — a new build either way), the atlas sprite, the enemy's link at a shorter `go`, the two-tap gesture as one drag; the wackiness the s88 log showed outside the tunnel — kings through portals, the swap as a free recall, a portal one rank shy of a king row (an ini-only fix: two rows off each king row — RULED AND BUILT 2026-09-20, the next paragraph) — is the designer's to rule on next. THE VERDICT CAME THE NEXT DAY (designer, 2026-09-20): "Alright this works pretty good" — Portals v4 is IN; the branch merges as it stands (the four portal commits, v2 through v4, on `claude/portal-mechanics-rework-0l17mj`; `main` had not moved and the merge is conflict-free).**

**PORTALS v4.1 — TWO ROWS OFF THE KING ROWS ✅ BUILT 2026-09-20 (designer, on the v4 verdict: "Let's make it so portals must be placed two spaces away from promotion zones instead of one.").** AN INI RULE, NO ENGINE CHANGE (the v4 paragraph's own note, ruled on): `variant.mjs portalIniKeys` emits the cast's `dropRegionWhite/Black` as ranks 1 + `PORTAL_ROW_MARGIN` … R − `PORTAL_ROW_MARGIN` with the margin 2 — ranks 3…8 on the 10×10 box, 3…6 on the selftest's 8-rank board, rank 3 alone on a 5-rank one; ranks 2…R−1 until then — the same for both colours, so a portal exit is never one push from a promotion (the easy queen is portal, push, push, two plies to answer); the engine's own floor stays the king row (its FEN-field parse drops an entry there — brief §4.7), the two-row rule lives in the deal where every placement guarantee lives (brief §4.2), and the engine tests' variants keep their one-row regions (they test the engine's rules, not the game's placement). THE NAME CARRIES THE MARGIN: `PORTAL_VARIANT_SUFFIX` is `__portals2` (rule 7 — `__portals` named the one-row deals; the two committed portal samples carry them and load as ever under their own recorded ini, and the analyzer appends every log's ini to ONE catalog, where a same-named variant with a different region would have been a silent no-op). THE PAGE NEEDED NOTHING: cast mode lights ffish's legal casts, so the two rows went dark on their own. Gates: selftest 50/50 headless (32 casts on the 8-rank deal board — ranks 3–6, none within a row of a king row — 31 links and 36 moves after the link, the fizzle boards a rank up), ui-smoke 375 ok (cast mode lights 33 squares on s59, none within a row of a king row), replay-smoke 76 ok (both portal samples load under their recorded `__portals` ini), test-barrier 165 (a portal deal's name `__portals2` and both regions `*3 … *8` on the box), test-logreport 61, test-portals-game 27; the engine is untouched, so its gates stand as recorded.**

**THE ICE ✅ BUILT 2026-09-20 (designer: "Time to implement a spell that creates a 3x3 ice patch. A few thoughts: 1. I intend for there to be oil and ice in the game, so they will likely share the slip logic. 2. I'm thinking when a sliding piece bumps into another piece on the ice, it should transfer the momentum. 3. I'm unsure how it should interact with portals… 4. Just like the other abilities we're testing, give each side one cast per game for now. Don't build anything yet, discuss first" — then the rulings, then "Sounds good, go ahead an build it").** THE RULES ARE THE DESIGNER'S (brief §4.9, in full): "a sliding piece should keep sliding until it's not on a slippery surface anymore. Or until it hits a wall or another piece" — a piece whose move ENDS on a slippery square glides on in its own direction and rests on the first square that is not slippery, a wall, a crate, the edge or a piece stopping it on the square before; only the square a move ENTERS matters (a rider's line runs over ice); MOMENTUM PASSES (a slider that bumps a piece STANDING ON ICE stops, and that piece slides on the same way, chained, either colour, kings included; a slide never captures); "Sliding pieces can fall into holes and die. So I guess we do in fact need to differentiate between holes and indestructible walls" — THE PIT IS ITS OWN GLYPH `_` to the engine now (a hole nothing enters by a move, that a sliding piece falls into; a crumble writes `_`; bedrock and the map's edge stay `#`; §4.8's one-glyph rule amended; old logs and crops keep loading through the ledger); "either king can slide and fall into a hole, just make a move illegal if ends with your king dead or in check, just like always" (your own king in a pit: illegal; the enemy's: a win on the spot, termination `pit`); a portal square is never slippery — a slide that reaches an EMPTY one is a landing out of the twin (swapping as ever), a piece on one an obstacle; "don't assume that every slippery patch is always going to be a 3x3 placed spell" — the engine keeps a bitboard of slippery squares of ANY shape (`~e5` entries in the FEN's trailing field), the cast one writer of it; a knight lands where it jumps; a capture on ice slides the captor on (en passant included); a pawn that STOPS on its promotion zone promotes (its own move to the piece named, a shoved pawn to the strongest); ONE SCROLL A SIDE (`I@e5`, one ply, never in check, on any non-terrain square of the board's two MIDDLE rows — ranks 5–6 on the box, so a patch never comes nearer than two rows to a king row — occupied or not); the gods leave slippery squares alone; nothing on the walk. THE ENGINE HALF is `engine/patches/ice.patch` (1,095 lines, +677/−21 across nine files; the EIGHTH patch, applied after portals-cast; `engine/README.md` § "The ice patch"): a SLIDE move type (10) encoded on the entered square, the physics (`slide_outcome`) recording every relocation as a `SlideStep` list in the state so undo is the state pointer (a portal landing's swap step recorded BEFORE the mover's — the oracle caught the clobber), `slide_attacks_king` judging legality and check on the virtual board, a rider's quiet move onto a run of ice generated ONCE at the last empty ice square before what stops it (pawns, kings and captures always), the slide evasions generated whole and filtered (a leaper's evasion onto ice took the evasion target — the oracle's second catch), `pseudo_legal` refusing a NORMAL-typed move onto slick with a direction, no en passant square after a slide, `st->slideEp` for a slide whose capture was en passant, SEE by the capture alone, `is_immediate_game_end` for a side whose king fell; the ice scroll a custom immobile piece (`customPiece1 = i:`, `iceScroll = i`, the cast rows its `mobilityRegion…CustomPiece1`, the drop on occupied floor allowed, `ice_patch(s)` the 3×3 ∩ board ∩ ~dead), `is_spell_scroll` keeping every scroll out of the extinction count and the eval; `_` parsed into `holeSquares` ⊂ hard ⊂ walls, `Zobrist::slick` / `hole` from their own PRNG. NATIVE FIRST, the oracle grown for the ice (`engine/forge/oracle.py`; `native_test_common.py` the shared driver, `native-test-ice.py` the fixtures, `ice.ini` the test variants, `apply-xsweep.py` the scratch sweep): 55/55 fixtures on the debug build with asserts on and on the release, 4,200 random positions (ice as a patch, scattered, most of a floor; pits, pairs, scrolls; 6×6 / 8×8 / 10×10) equal to the oracle on perft-1 move sets and perft-2 per-move counts under `xsweep`, perft 4 on nine fixtures under asserts, the portal fixtures 62/62 + sweep 71/71 on the ice build, ice-free boards node-identical to the seven-patch build on five boards. THE GATE, both binaries rebuilt from the clean pinned trees (the glue's seven layout bytes each, the worker identical; the wasms +18,109 / +19,403 bytes): test-ice-ffish 53/53 (a 3,932-move check-flag sweep), test-ice-engine 79/79 (perft 3 pinned to the native build on thirteen fixtures, the 10×10 duel shape perft 2 = 4,453, the search finding the shove into the pit), test-portals-ffish 78, test-portals-engine 146, test-hammer-ffish 27, test-hammer-engine 25, test-ffish 19, test-engine 7, regress + regress-ffish identical, xcheck, stack-regress 5, search-identity node-for-node at d12, depthcap d22 110/110 (slowest 1,533 ms) and d60 30/30 — THE CAP STAYS AT d22. THE GAME HALF (`play/README.md` § "The ice spell"): `play/js/ice.mjs` — `slideOutcome(fenBefore, uci)` mirrors the physics on the grid (the mover's step, then each shoved piece's; a fall named by its pit; a landing with its swap; the promotion where a pawn stops) and `slideAliases` names the RESTING squares — held to ffish's own board after every slide by `phase0/harness/test-ice-game.mjs` (42: the engine gate's fixtures and 1,142 random slides) and the selftest; `fen.mjs PIT` `_` in `isTerrain` / `isWall`, `parsePortalField(...).slick`, `slickSquares` (the field's `~sq` less the portal squares), `withSlick`, `castLetter`; `variant.mjs ICE_SCROLL` / `iceIniKeys` / `iceCastRanks` (the two middle ranks) / `spellIniKeys` (ONE value line for every scroll) / `spellPocket` (`[IOOioo]`) / `dealVariant({ ice })` under `__ice` (rule 7), threaded through `dealMatchup`, `planBox`, `planBarrier`, main.mjs `iceOn()` (`options.ice`, Options → Spells "Ice spell", `?ice=off`); `world.mjs` spells a HOLE `_` in every crop FEN and stage grid and reads it back, `crumbleFilter collapseFen` and the Director's terminal crumble write `_`, `board-ui classifyCell` reads it as a hole ledger or not, the report prints it `O`; duel.mjs records `slide` on every state, `castKind` `ice`, termination `pit`, `mustLink` specific to the portal cast; the Director's `blocked.slick` (no landing on, no pit under, a slippery square); main.mjs's ❄ Ice button on the portal's cast path (`app.castMode` the spell's KIND, `SPELLS`, `castTargets(kind)`), the resting squares lit and tapped through `moveAliases` (Portals v2's exit alias folded in), the move animated leg by leg (`canvas-board animateSlideChain` — the chess move, the glide easing to a stop, each shoved piece in turn, every finished piece parked until the chain is over, a fall sinking into the pit), the log's words (`logreport slideWords`), the arrows to where a piece rests (`lastMoveArrows` with a short arrow per shoved piece, `slideRest` for the hints; the analyzer alike), the end overlay's `pit`; `canvas-board setSlick` + `#iceTile` (the flagstone pulled toward `ICE_TINT` with a sheen and a rim, a paint-time composite per theme / variant / shade, no atlas row). Gates: test-ice-game 42, selftest 51/51 headless, ui-smoke 318 ok (THE ICE block: the button with one scroll, cast mode lighting every non-terrain square of ranks 5–6, a tap on a wall leaving it, the cast icing the floor of its 3×3 and spending the scroll, the record's `ice` cast and the log, the ice pixels on the patch and none off it, and on s73 — its cast rows open floor — a king stepping onto the patch and sliding two squares, played through the lit resting square, the state's slide equal to the grid's prediction, the piece at rest after the commit, the log's "Kb5 — slides to b7"; `?ice=off`), facing-walk 108/108, replay-smoke 76, test-logreport 61, test-world 132, test-barrier 170, test-portals-game 27, the other Node gates unchanged (test-armygen on the overlaid pair). THE PHONE VERDICT CAME 2026-09-24 (designer, with the log: "Looks like it works as far as I can tell") — THE ICE IS IN, and the log is THE FOURTH COMMITTED SAMPLE, THE FIRST ICE DUEL (`replay/samples/dck-log_vaults-2-t24_s3571496125.json`, `?sample=4`: vaults-2 at walk turn 24, the enemy's initiative, restless, Android Firefox, 68 plies, 3 quakes of 65 rolls, 6 undos, 0 anomalies, 0 recovered searches, 1-0 by checkmate at ply 68). WHAT THE RECORD SAYS: BOTH SIDES CAST THE ICE — the player's `I@d6` at ply 26 ices the eight floor squares of its 3×3 (e5 is a wall and takes none), the enemy's `I@b6` at ply 39 OVERLAPS it (fourteen slippery squares to the end); a pawn's DOUBLE STEP d3–d5 onto the patch slid on to d8 at ply 34 (three squares, the log's "slides to d8"), the enemy's rook c9–c5 was stopped on c5 by the pawn on c4 ("stops on c5"); the enemy cast its portal pair f4–c8 after the ice, in one turn (the half, the player's frozen pass, the link); and BOTH KINGS HAMMERED — the enemy's three times (K*e10, K*f8, K*g8), the player's once — where the third sample's enemy never did. test-logreport 71 and replay-smoke 88 ok read the sample (the casts on the timeline, the slides in words, the field cast by cast, the hammers; the analyzer painting the ice on the patch's empty squares from the field and none on the wall or beyond, the slide's arrow ending on d8, the rook's on c5, a played cast as the bare frame at its edge — gold for the player's, red for the enemy's — the pass drawing nothing, the pair's orange rings under the frame). Held over: oil, other ice shapes and floors of ice, the ice as an upgrade, a spray of frost on the cast, a mark where a piece fell.**

**THE SPELL GLYPHS ✅ 2026-09-21 (designer, on the ice build: "On move hints, there's just a square outline for both portal and ice. How am I supposed to know what spell it's suggesting?"; on the first cut, the same day: "For the ice patch hint, outlines around all 9 tiles is way too loud. Just the one center square is fine. Also the icon could use some work" — with a reference snowflake, six-armed and hexagonal with forked tips and a hollow hub).** A cast hint was one frame for either spell. Now every PROPOSED cast — a hint, a cast in the analyzer's numbered PV lines, where the board does not show the spell yet — is its square FRAMED AT ITS EDGE (`#frame1` at inset 0, where the target marks sit a pixel in) with THE SPELL'S GLYPH inside (`play/README.md` § "The spell glyphs"): `pixelarrow.mjs PORTAL_GLYPH` a 10×10 ring, `ICE_GLYPH` an 11×11 snowflake drawn to the reference (the vertical arm with forked tips, the four arms at thirty degrees with a barb each, a hollow diamond hub), `drawSpell` stamping either in the arrow's colour with a ONE-PIXEL DROP SHADOW (`SPELL_SHADOW`) and NOT the arrows' eight-neighbour halo — the halo fills the gaps between a flake's arms and made the first cut a black tile with a gold star (judged on a scratch sheet at ten times scale: the halo, a four-neighbour halo, no halo, the shadow, in the three rank colours); the centre square alone for the ice (the first cut framed the 3×3 the patch would freeze — "way too loud"); a PLAYED cast (the enemy's own under the red last-move mark, the analyzer's cast ply; arrows flagged `played`) keeps the bare frame, because the board shows the spell itself there and the first cut's glyph over a pair's rune ring hid the colour that says whose pair it is (replay-smoke's ring counts caught it); the hint list wears the glyph before the SAN (main.mjs `spellIcon`, `.hint-glyph`), and the enemy's cast and the analyzer (`lastMoveArrows` / `moveArrow` with `played`; `pvArrows` — a cast in a line numbered like the arrows, a pass drawing nothing) name the spell off the scroll letter. Gates on the second cut: replay-smoke 76 ok / 0 failed (its ring-pixel counts at plies 2, 3 and 5 of the portal sample are the played-cast guard — they failed on the first cut's glyph over the ring), the ICE block's two glyph checks green on a scratch run of the block (28 ok / 0 failed: the flake's top tip and hub in the rank-1 gold on b5 of s73, the hub hollow, the tip's black shadow, the frame's corner gold with floor inside it, no frame on any neighbour, the list `1:ice 2:-` with its text unchanged; the ring on g3 of the `?ice=off` page gold with its shadow and hollow, framed at its edge alone, `1:portal 2:-`; hints off clearing both); then the full re-run on the second cut: ui-smoke 353 ok / 0 failed (the count moves with the random driver; the first cut's run was 383) and selftest 51/51 headless; no Node module changed (pixelarrow.mjs is the board's and the page's alone), so the Node gates stand as recorded. Crops at a phone's dpr 3 (the board with an ice, a portal and a plain hint; the hint list) went to the designer with each cut. THE VERDICT CAME 2026-09-24 with the first ice duel's log (designer: "Looks like it works as far as I can tell") — the second cut is IN; the branch (the ice + the spell glyphs, `claude/beautiful-einstein-lidvel`) merges as it stands, `main` unmoved.**

**THE DECK — SPELLS AS CARDS, STAGE 1 ✅ BUILT 2026-09-25 (designer: "Could we have spells as cards drawn from a deck? I know FSF doesn't work with random elements and hidden information, so what if we just made both decks and hands fully visible to both players… Don't build anything yet, just discuss" — then the rulings, then "Is there anything we want to build this session to get started?"; brief §4.10 carries the rulings, the measurements and the staging IN FULL — read it before touching a card).** THE RULINGS: a HAND OF FOUR, drawn up to at the start of each turn, an option to spend the turn discarding and redrawing; TWO DECKS, starter decks once spells exist, cards earned in duels and found in chests, a shuffle and a fresh hand at every duel (the deck is the run's collection), deck strength scaling with army size; a cast is the caster's move — free casts are buildable on the portal cast's frozen pass and are NOT the rule; META CARDS (Reveal: the engine's best three lines this turn; Undo: take back your last move and the enemy's reply) are the player's alone, cost no move and never reach the engine, and the enemy's deck holds only what the engine can search; lethal by default (shocks and burns kill; lesser electric spells stun in an area; an ice spell entombs a piece in a block until a hit frees it); destruction as HITS with areas (intact → cracked → floor, bedrock immune, pieces untouched); elements as MATERIALS with a transition table and FIRE that spreads once per full turn to orthogonal fuel; the gods' STIRRING to be retired by cards and their CLOSER kept, measured before anything goes. **A STANDING RULE, the designer's, emphatic: NO CARD IS EVER VALUED BY A NUMBER** — "When the engine chooses to play a card, it should do so because it's objectively the best move, not because of some arbitrary number we picked… DO NOT artificially value spells just to make the engine play suboptimally. That'd be completely back asswards." A scroll's piece value stays 0; a cast rate is tuned by a spell's RULES (range, area, tempo, copies), never by a value (a per-card value dial was proposed and withdrawn the same session; §12's "no engine handicapping" applies to cards in full). MEASURED FIRST (a scratch self-play driver on the vendored pair, engine vs engine, gods off, the ice and sledge deals at depth 16 / 2 s and one game at the live depth 22 / 10 s): at value 0 the engine casts on about one castable turn in twenty, every cast within a pawn of its best plain move (most within a quarter pawn) — it casts when a spell is worth about a tempo; the side BEHIND casts most and early (the phone logs: the enemy opened with a portal in two of three duels while two to nine pawns down); situational cards are held (the ice on empty middle rows, twenty turns); casts are listed once per card KIND in hand, not per copy (a fat hand's root was the same 68 casts as the shipped hand's); 68 casts at the root cost about 1.5× the time to depth 16 and about two plies at ten seconds — HAND SIZE IN KINDS is the engine's cost. BUILT (no forge; `play/README.md` § "The deck"): `play/js/deck.mjs` — the catalog (`CARDS`: ice, portal, reveal, undo), `STARTER_DECKS.adept` (3 portals, 3 ice, Reveal, Undo), `enemyDeck` (width − 1 spells by seed), the seeded shuffle off the deal's seed (`deckSeeds`), THE POCKET IS THE TRUTH for spells (a card's scrolls ARE the holdings; `spellHand` rounds a half-spent portal up to a card; the meta cards sit in the deck state's `meta`), `openHands` / `drawUp` / `mulligan` / `deckRecord` / `parseDeckParam` (a hand-built `?deck=` list deals in its own order); `duel.mjs` — `opts.decks`, the REFILL for the side to move at each turn's start (`#refill`, inside the new `#afterPly` after the quake phase: a drawn spell is a bare position for the engine, a meta card joins the deck state; never on the frozen side's pass or the link ply; `drew` on the state), `deck` on every state, `metaPlays` in `RECORD_ARRAYS`, `playMeta` (the top snapshot's decks and lens follow, so an undo to that state keeps the card spent and the play on the record), `hands()`, `canMulligan()` / `mulligan(mover)` (a `--` ply, the turn flipped by a bare position, `cast: 'mulligan'`, the gods read a cold ply), the snapshot's `decks` restored by an undo; `armygen.dealMatchup` / `barrier.planBox` take `pocket`; `run.deck` (`dck-run/6`), the enemy's deck on the walk from ITS seed; the log's `decks` + `metaPlays`; main.mjs § THE DECK — Options → Spells → Deck (the starter, or "Everything, no deck": the stress-test set, which the older smokes pin with `?deck=off`), `?deck=off|adept|<list>`, the spell buttons counting CARDS, `btnReveal` / `btnUndoCard` / `btnMulligan`, `#deck-line` (the pile in order), the enemy's hand and pile in its bar, Reveal through the hint probe (`revealOn()` beside `cheatHints()`), the Undo card through the cheat undo's path spent in the restored state, the redraw, the log's draw lines, `__DCK.deck`. GATES: test-deck 50, test-deck-duel 36 (the vendored pair in Node — the refill, the frozen pass and the link ply, the meta plays through an undo to their own state, the mulligan and the engine moving on, no decks = the old controller), ui-smoke 446 ok / 0 failed (THE DECK block + the older blocks on `?deck=off`), selftest 51/51 headless, replay-smoke 88 ok, test-barrier 170, test-logreport 71, test-world 132. THE INSTRUMENT: `phase0/harness/deck-stress.mjs` — engine against engine on the canon controller with two decks (per card kind the cast count, the median ply, the share cast by the side behind, the cards dead in hand; per side the casts per 100 plies and the share of castable turns cast); its first batch, `phase0/results/deck-stress/adept-vs-width3-d8.jsonl` (four games on s59, the Adept deck against a 3-wide enemy, depth 8 / 300 ms, gods off): the deck side cast on 5% of its castable turns (2.4 casts per 100 plies, ice at a median ply 69, portals at 43, eight cards dead in hand at the ends), the enemy on 6% (every one of its casts while behind, none left in hand), three strips and one ply cap. THE STAGING (agreed — "possibly a bit ambitious to build all in one go… get a healthy variety of spells before we really even attempt to nail down the balancing"; NUMBERED 2026-09-25 as PHASE 3's sub-phases, brief §10 — Phase 3 "the loop" opens with the deck, the collection it always was: 3.0 the record ✅, 3.1 the deck ✅, 3.2 THE CARD UI ✅ BUILT the same day (designer, on the proposal's four calls: "Looks good. Agree with all 4 calls" — the paragraph below), 3.3 terrain edits, 3.4 timers + piece states, 3.5 materials (a without fire, b fire), 3.6 the run's economy, 3.7 balance by rules + the gods to a preset, 3.8 floors, 3.9 piece identity): 2 TERRAIN EDITS, the first forge (hits + shapes, Sink, Raise, Harden, the sledge as an ENCHANTMENT flag retiring the always-on setting, a per-type drop region for Reinforcement, Golem by ini); 3 TIMERS + PIECE STATES (Stun, Entomb, Ward, melting ice, Palisade); 4a MATERIALS without fire (Water, Oil, Lightning); 4b FIRE (the tick, lethality, scorch, barrels); 5 Gust, chests, loot, deck strength by width; later PIECE IDENTITY (the twin). Balance is RULES only, after 4b, read by deck-stress and the god lab with decks. Open, the designer's: a hit freeing an ice block in one or two; the lethal fire cast its own card or the ignite card with a body count; the first table's four transitions. THE PHONE VERDICT IS THE GATE.**

**THE CARD UI — PHASE 3.2 ✅ BUILT 2026-09-25 (designer, on the deck's first card row: "These little buttons aren't gonna cut it. I wanna take notes from other card wielding games like mtg arena and hearthstone. Probably cast spells by dragging them from the hand to the field, that kind of thing"; on the proposal and its four calls — the drag preview tints an area card's whole shape, the enemy's played card holds for a beat, the redraw is a tap on the deck stack behind a confirm, the portal's second square is a tap with cast mode held or the card dragged again — "Looks good. Agree with all 4 calls"; brief §4.10 "The card UI", `play/README.md` § "The card UI" the record).** THE HAND IS A FAN under the board (`#hand-row`; `play/js/cards.mjs` the pure half — `fanLayout`, the art, the gesture, `castArea`, the words — main.mjs § THE CARD UI the page): every copy its own card, a face a CSS frame around PIXEL ART — a spell card's art IS its hint's glyph off pixelarrow (one drawing, rule 19), the meta cards' two drawings of their own (`REVEAL_GLYPH` an eye, `UNDO_GLYPH` a counter-clockwise arrow) — with the name, one line (`CARDS[k].short`) and a COST PIP (`cost`: a spell is your move, a meta card free); the playable ones rimmed (the spell blue, the meta gold, the enemy's red), the rest DIMMED; the outer cards tilted and dropped for the arc, `FAN_INSET` keeping a corner inside the row; a LIFTED card is cast mode for its kind. ONE GESTURE PATH (`bindHand`, pointer capture; `gestureStart/Move/Hold/End`): a TAP lifts a spell card into cast mode (the tap on the board casts, as ever — every older smoke's path stands) or plays a meta card; a DRAG past `DRAG_SLOP` 8 px lifts a GHOST (a fixed-position DOM card 24 px above the finger with a tooltip) and lights the legal squares, the square under the finger the board's new `hover` mark (a frame at its edge in the spell blue) and an ice card's 3×3 on the floor its `area` marks (a translucent tint under the pieces — canvas-board `setMarks` gained both; the drop test is `squareAtPoint`, so RULE 18 IS UNTOUCHED: nothing new aligns with the floor), a release on a lit square casting through `playPlayerMove` and a release anywhere else snapping the card back with nothing played; a LONG PRESS (`LONG_PRESS_MS` 450) opens THE READER (the card at 8× art, its full text, its cost, its line — the portal's open half asks for the link); the piles beside the fan (the deck a stack with its count, the spent pile), a tap on the deck opening THE DECK SHEET (the pile in order, the spent pile, THE REDRAW as Redraw → Confirm → `playMulligan`, Cancel withdrawing it — a turn never spent by a slip); THE ENEMY'S HAND FACE-UP as mini cards in its bar (`#enemy-cards`; its text `#enemy-bar-text`) with its deck a mini stack (a tap opens the sheet on its pile, no redraw); THE PLAYED-CARD BEAT (`enemyCardBeat`, awaited in `onMove` before the spell paints, on the enemy's ice cast and the HALF of its portal cast): its card flies from its mini to the cast square at 1.2×, holds half a second, fades; THE DRAW slides a new card in from the deck stack. The spell buttons, the meta cards' buttons, the Redraw button and the deck line are GONE (`?deck=off` shows the two spells in hand as cards and no piles). `__DCK.cards` is the surface. GATES: `test-cards.mjs` 54, ui-smoke 438 ok / 0 failed (THE CARD UI block through REAL POINTER EVENTS — Playwright's mouse: the drag, the frame and the tint and the tip over the target, the cast on release, the snap-back off the board, the long press, the Undo card's tap, the enemy's minis and sheet, the beat on a motion page; the portal, ice and deck blocks moved onto `K.cards.tap`), selftest 51 headless, replay-smoke 91 ok, test-deck 50, test-deck-duel 36, the other Node gates unchanged; five phone-ratio shots went to the designer. THE GLUED READYOK, FIXED IN THE TRANSPORT the same day: three of four full smoke runs died on the engine transport glue on record since 2026-09-12 (`bestmove c4b5 ponder e7d8readyok` as ONE message — the search thread's bestmove and the main thread's readyok with the newline between them lost — so `isready` never saw its `readyok`; the two earlier records left engine.mjs untouched and reran), and a page's boot hung the same way twice; `play/js/engine.mjs` and its Node twin `phase0/lib/load.mjs` now split a glued `readyok` off the end of any line before a listener reads it (`splitGlued`: no UCI line ends in "readyok" but "readyok" itself — the engine binaries and the rules untouched); and a PAGE BOOT HANG the glue does not explain — three long runs saw a page never reach 'playing' in 120 s (a page with no card in it, once the ninth of a run) while twelve isolated boots of the same URLs took 3.5 s each — and the sixth run found the reason a reload could not help: the page's TARGET WAS GONE ("Target page, context or browser has been closed" — a renderer lost mid-run, the same URL booting in 3.5 s on a fresh page) — (45 boots in one Chromium reproduced it at the 25th, the browser itself alive; /dev/shm is 16 GB and empty, memory free, no orphan process) — so ui-smoke's `bootWait` now REPLACES THE BROWSER: a page that has not booted in 90 s is closed with its browser, a fresh Chromium is launched (`--disable-dev-shm-usage` beside `--no-sandbox` now — 45 of 45 boots in one browser were clean with the flag where the run without it lost its 25th, one sample each way) and the page re-made, re-hooked and re-navigated in it, the retry counted on the summary line (never hidden; a run with retries is on record as such); the clean run needed no retry. Held over: a drag from the first portal SQUARE to the second (dragging the card again links it today), the wide layout's fan (in the panel column, `--card-w` 92 px), a sound. THE VERDICT CAME THE SAME DAY (designer, with a log: "Pretty funky but it works. Looking forward to more card variety.") — THE CARD UI IS IN, and the log is THE FIFTH COMMITTED SAMPLE, THE FIRST DECK DUEL (`replay/samples/dck-log_s77-the-smithy_s210339940.json`, `?sample=5`: s77 flipped on the arena page, Firefox/Windows, the Adept deck against the starter's six spells, 85 plies, 8 quakes, 5 undos, 0 anomalies, 1-0 by checkmate; a Reveal at ply 10, six draws; `logreport.mjs` prints a `decks` header line and marks draws, redraws and meta plays on the timeline — test-logreport 79, replay-smoke reads it). WHAT THE LOG SHOWED, THE DESIGNER'S QUESTION ("The enemy portal placement is pretty weird. What's up with that?"): the enemy cast its whole deck from behind (three ices, three portal pairs at −9 to −29): a3–d3 deep in the player's camp, an ADJACENT pair g5–g6 (its line dropped a bishop through it one square), j8–d4 six plies from mate — every square inside the cast rule, the field the engine's own: the no-values rule at work — a linked pair is two BODIES the opponent can never remove, and a losing engine casts the least-bad move its eval finds (a two-square wall beside the ice, not a transport). If it should read differently the fix is a RULE of the spell (a minimum spread between a pair's squares — the link's drop generator in portals-cast.patch, a forge; or a cast kept out of the opponent's half), never a value. The designer's to rule on.**

**PHASE 3.3 — THE ENGINE'S DECK AND THE TERRAIN INTERPRETER ✅ DISCUSSED 2026-09-25 (docs only — nothing built; the record is brief §4.10 "Phase 3.3", the spec the forge opens on — READ IT BEFORE TOUCHING THE ENGINE, deck.mjs OR duel.mjs's refill).** THE CORRECTION: stage 1's draw OUTSIDE the engine (duel.mjs `#refill` between plies, "nearly as good as a draw inside the engine") was the previous session's judgment, not the designer's, and it is OVERRULED — designer: "Does the engine still understand that it will keep drawing cards? … The engine playing OPTIMALLY is non-negotiable"; "I thought I was clear on the last session that the entire card and deck mechanic needs to work at the FSF engine level"; "I intend for there to be a FUCK TON of cards. Expect full decks of entirely unique cards." FSF holds a deck without breaking (a visible pile with a fixed order is a deterministic perfect-information alternating game — no hidden information, no randomness in the tree, no multi-move turn; pieces entering the hand mid-move, non-board counters in the state and the key, and irreversible plies are all things crazyhouse, gating, three-check and our own portal patch already do). THE DESIGN, 3.3a — THE DECK IN THE ENGINE: the remaining pile per side as card IDs in the FEN's trailing field; the draw at the end of `do_move` for the side about to move (the game's refill exceptions kept: the frozen pass and the link ply draw nothing; `undo_move` off the state, the slide's idiom; `#refill` RETIRES, the record reads a draw off the FEN); THE HAND IS SLOTS, NOT KINDS — eight reserved letters shared by both colours, each bound per colour to a catalog ID, bindings and pile pointers in the copied state and hashed into the key (a piece type is one FEN letter, so binding kinds to letters would cap a duel at ~20 kinds — the slots cap only distinct kinds IN HAND, and a hand is four); the defs of every card in either deck declared in the deal's variant ini as `card<ID> = effect:shape:targeting` (the name a hash of the ini — rule 7; the log records the ini already); the mulligan an ENGINE MOVE; Reveal and Undo as BLANKS in the engine's model of the player's hand; casts, mulligans and draw-carrying plies EXEMPT from late-move reductions; the two existing spells as the first cards (the portal keeps its letter and machinery, the ice becomes a def), gated on the committed samples and re-measured with deck-stress. THE FORESIGHT ON RECORD: within the horizon exact (every line carries its exact draws; ~eleven of the engine's own moves at depth 22; a card at pile position p is ceil(p/4) mulligans away; theoretical reach six to eight mulligans for an instant payoff, practical three to six; every mulligan is a free move for the opponent, so the engine will see a deep card and mostly decline it, correctly; a cast is a one-card dig at no cost), beyond it nothing — the leaf prices the board and the material in hand, never the pile, never spells (the no-values rule); the only route past it is MEASURED card-class values tuned by self-play, a reopening of the no-values rule, NOT before the exact engine is measured. "YOU WIN" — THE HORIZON INSTRUMENT (designer's own test card): effect `win`, an immediate game end scored as mate, cast on the caster's king, test-only, out of check; measured by the ROOT SCORE and PV (mate in k+1 through the mulligan line), one copy on the engine's side vs a chess-playing opponent, deck length 8…44 by fours = one to ten mulligans, at the live and the lab go — THE HORIZON CURVE; the MIRROR (the card on the other side, that side scripted to mulligan every turn); "always at the bottom" = last in a fixed-order `?deck=` / `--deck` list; a permanent gate fixture beside the mate fixtures. NO CARD IN CHECK — A STANDING RULE, the designer's, emphatic: "The only cards you are legally allowed to play while in check are Undo and Reveal. No exceptions." — every cast, every piece drop from a card (Reinforcement's pawn: "I do not give a fuck about the rules of crazyhouse" — card pieces excluded from the evasion drops by name), the mulligan, the redraw and the win card generate out of check only; the evasions untouched; the interposition proposal WITHDRAWN; a check denies the opponent every card while it lasts (forcing play is card denial; perpetual check is the stalling case the gods already handle). 3.3b — THE TERRAIN INTERPRETER: `effect:shape:targeting` — SHAPES as picture strings (`xxx/xox/xxx`, `xox`, `x../xxo`; `o` the anchor under the finger; NO ROTATION on cast — an L's four orientations are four cards; clipped at the edge; ~thirty starter shapes: the monomino, dominoes, trominoes, the nineteen tetromino orientations, the 3×3, the plus — designer: "Every AOE card could have a ton of shape and size varieties", the ice included); EFFECTS `hit n` (intact → cracked → floor, bedrock immune, pieces untouched, the crate ledger), `ice`, `wall` (RAISE AND GOLEM ARE RETIRED for "AOE shield/wall spells of various shapes"; a raised wall is a `*`; the timed indestructible wall is 3.4's), `pit` (Sink), `harden` (PETRIFY ONLY — `*` → `#`; "I don't want to see a chair turn into an actual wall": the mend step waits for 3.5a's MATERIAL BIT, wood vs stone on a `^`, never a second glyph), `sledge` (the enchantment flag per colour, hashed; retires the always-on setting AND the walk's hammer), `win`; REINFORCEMENT a real pawn in hand with a per-type drop region key (the stock per-type region is the mobility region and would pin it); TARGETING `any` / `near` (the anchor within a king's step of one of the caster's pieces, the king included) / `margin:2` (every cell two rows off both king rows — the ice's rule generalized) / `king` / `ray` (from the king, eight directions, piercing to bedrock or the edge; THE LANCE = `hit 1` + `ray`; designer: "Def wanna experiment with a wide variety of targeting types"); THE RULES OF EVERY CAST: a cast that changes nothing is NOT GENERATED (the ice's re-icing pass closes with it), CHECK BY DEMOLITION is real (the crate-to-floor step judged on the virtual board for legality and check), IMMUREMENT allowed (stalemate is a loss), zero values (a scroll 0; the Reinforcement pawn a pawn). THE ORDER: forge one = 3.3a (the existing spells, the samples, deck-stress, the win curve), forge two = 3.3b, then two page PRs (the hits, ice shapes and builders; then sledge, Reinforcement and the win card in the test decks); the page's share mostly reuse — the cast path per slot, the drag preview's tint, the crack / burst beats and debris, the holes ledger, the bedrock paint — plus card faces GENERATED from the def (effect glyph + shape mini-map + targeting badge), the words, the record's edits and draws. PROPOSED, PENDING (§11): the shaped-cast HINT (anchor framed + faint shape tint, on a crop), the RANDOM starter as the phone's default beside Adept and SAPPER, systematic names, `?deck=off` staying the old three spells, a card never a piece until played (the strip rule). Supersedes in this file: the deck paragraph's staging line "2 TERRAIN EDITS … Golem by ini" and its refill-outside-the-engine description, both history now.**

**PHASE 3.3a — THE DECK IN THE ENGINE ✅ BUILT 2026-09-26 (the first forge; designer: "Go ahead and get started on 3.3a … I want an engine that actually sees and understands the deck"; `engine/README.md` § "The deck patch and the deck-search patch", `play/README.md` § "The deck in the engine", brief §4.10 "Phase 3.3").** THE FEN IS THE DECK: a hand is card SLOTS in the holdings (custom immobile pieces `s..z`, one letter per copy, identical cards sharing a slot), each bound per colour to a card ID by the trailing field (`S=w1`, `+` the slot whose portal half is open), the pile the field's `w|2.1.101` (top first, both piles visible), `!w` the winner; the ENGINE draws for the side about to move inside every move that hands it the turn (not the frozen side, not the link ply), offers the MULLIGAN as the move `@@@@` (SAN `redraw`), casts a card as its slot's drop by the kind the deal declared (`card<ID> = portal|ice|win|meta`, `handSize`, `cardSlots` — variant.mjs `deckIniKeys`, the name `__deck4_1i_2p_101m_102m`, rule 7), NEVER casts in check (the meta cards are blanks the game plays outside the grammar), and searches through both piles. THE DIG IS NEVER PRUNED (deck-search.patch: the mulligan and a win cast skip move-count pruning, Step 13 and LMR; a win cast is ordered first) — measured on a counterplay-free board: the win card under 0/4/8/12/16/20/24/28 cards is mate in 2…9 found at depth 3/5/7/9/12/13/22/23, eight mulligans deep in 9.6 s natively, and THE MIRROR (the deck on Black, White to move) reads the mate against it at depth 4/6/8/17/19/20/22/24 — the engine sees the opponent's dig too (`engine/forge/horizon.py`, `--mirror`). TWO ENGINE FINDINGS on the way: (1) the pile pointer must be A COUNT OF THE REMAINDER — a search's root is re-set from `pos.fen()` (the remainder alone) with the setup state copied over it, so a drawn-count pointer pointed past the re-parsed pile and the mulligan vanished at every root reached by moves (D13 in the fixtures: the root by moves equals its FEN); (2) A CARD IN HAND IS WORTH ZERO — FSF's psqt gives every piece in hand a flat (35,10)×(1+!slider) on top of its value, which had priced a value-0 scroll at 35–70 cp since the portal patch (the "small in-hand bonus" on record was not small); it is SCORE_ZERO for every spell type now, the legacy scrolls too — the no-values rule, in the engine. ONE RULE CHANGE beyond the deck: NO NULL CAST — an ice cast whose 3×3 would ice nothing is not a move (legacy scroll too; four ice perft pins and the 10×10 shape's re-pinned). THE GAME (`deck.mjs` rewritten around the FEN — the catalog's `id`/`engine` per card, ice 1 / portal 2 / reveal 101 / undo 102 / win 200; the opening deal `dealDeckFen` by the engine's binding rule; every reader off a FEN: `handOf`, `pileOf`, `spentOf`, `cardOfCast`, `castUci`, `removeCard`, `deckRecord`; `fen.mjs parseDeckField` / `withDeck`; the deal takes `decks`; duel.mjs `#refill` GONE, `mulligan()` pushes `@@@@`, `playMeta` rewrites the hand, `lastMove.card`, termination `win-card`; main.mjs casts through `castUci`, `deckView`; the analyzer off `st.card`); THE YOU WIN CARD (the designer's horizon instrument: cast on your own king, the game is over; test-only, `?deck=` lists may name it; a star glyph). GATES on the rebuilt pair (ten patches now — rule 15: dead-squares, thread-stack, portals, wall-kinds, hammer, portals-body, portals-cast, ice, deck, deck-search): the oracle grown for the deck, fixtures 55, 900 random positions + xsweep, perft 4 under asserts, identity IDENTICAL on scroll-free boards, test-deck-ffish 39, test-deck-engine 58, every older engine gate, depthcap d22 110/110 (slowest 1,392 ms) d60 30/30 — THE CAP STAYS AT d22; selftest 52/52 headless, test-deck 65, test-deck-duel 46, replay-smoke 91, test-logreport 79, test-cards 57, the Node gates unchanged, ui-smoke 427 ok / 0 failed on the final build (436 on the run before the controller's spent-deck fix below; one run between them stalled on a lost renderer — the flake on record, its boot retry logged — and is no count; its DECK and CARD UI blocks read the slot holdings, the engine's redraw and THE FAN IN SLOT ORDER — the draw order at the start, where stage 1 fanned the spells first; the run also caught a quake's marks wiped by the player's FORCED PASS between the enemy's half and its link — onMove keeps them through a pass now, the pass is the game's ply). ONE MORE FINDING, from the instrument: A DUEL WHOSE DECKS ARE BOTH SPENT CARRIES NO DECK FIELD (an empty pile and an empty slot emit nothing, and a redraw near the pile's end discards the blanks too and draws only what is left, so a hand can empty) — the controller's `hasDeck` reads `decks0` as well as the FEN, `hands()` is then two empty lists, and `deck-stress.mjs` reads a cast's card off the slot bindings and counts the engine's redraws. MEASURED (the stage-1 batch's settings — s59, Adept vs a width-3 enemy, depth 8 / 300 ms, gods off, four games; `results/deck-stress/adept-vs-width3-d8-engine-deck.jsonl`): the deck side cast on 12% of its castable turns (2.8 casts per 100 plies — 5% and 2.4 on stage 1's batch), its portals at a median ply 11 and its ice at 35 (43 and 69 before), 4 spells dead in hand at the ends (8 before), and ONE REDRAW: at ply 25 of game 2 the engine threw back two ices and both blanks to draw the last portal on its pile and cast it eight plies later — the dig, in ordinary play at depth 8; the enemy (two spells) cast on 10% of its castable turns (6% before), every cast while behind, nothing left in hand, no redraw; 0 of 583 searches at the time bound; results a checkmate, a strip and two ply caps — and game 2 ended with both decks spent, the position the controller now reads through `decks0`. NEXT: 3.3b THE TERRAIN INTERPRETER (the second forge), then the page: THE HINT FOR A SHAPED CAST is RULED (designer 2026-09-25: the anchor outlined plus the shape as a faint tint, AND "outline the card itself in gold/silver/bronze for further clarity" — the card in the fan wears its hint's rank colour). Not yet played on the phone.**
...
- `play/js/cards.mjs` — THE CARD UI's pure half (Phase 3.2, 2026-09-25;
  brief §4.10 "The card UI"): the fan's geometry (`fanLayout`), the card art
  (a spell's off pixelarrow, the meta cards' own eye and arrow, `stampGlyph`),
  the gesture split into tap / drag / hold, the drop preview's squares
  (`castArea`: the ice's 3×3 on floor alone), the words. main.mjs § THE CARD
  UI is the page (the fan, the ghost, the reader, the deck sheet, the enemy's
  minis, the beat). Node gate `test-cards.mjs`; ui-smoke's CARD UI block.
- `play/js/deck.mjs` — THE DECK (2026-09-25; brief §4.10), IN THE ENGINE
  since 2026-09-26 (Phase 3.3a; `engine/patches/deck.patch`): THE FEN IS
  THE DECK — the catalog (ice, portal, Reveal, Undo, the test-only You Win)
  with each card's engine ID and kind, the Adept starter, the enemy's deck
  by width, the seeded shuffle, THE OPENING DEAL into the start FEN's slots
  and piles by the engine's own binding rule, the deal's declaration
  (variant.mjs `deckIniKeys`), and every reader off a FEN — the hand, the
  pile, the spent cards, what a cast casts, what a ply drew, a meta card
  taken out; the engine draws at every hand-over and deals the mulligan as
  the move `@@@@`; `deck` on every state and `metaPlays` on the record; the
  run carries `deck`; main.mjs § THE DECK is the card row. Node gates
  `test-deck.mjs` + `test-deck-duel.mjs`; `deck-stress.mjs` the instrument,
  `engine/forge/horizon.py` the win-card curve.
- `play/js/ice.mjs` — THE ICE ON THE GRID (2026-09-20; brief §4.9;
  engine/patches/ice.patch): the slide's physics mirrored for the page —
  `slideOutcome` (the mover's step, then each shoved piece's, a fall, a
  portal landing, the promotion where a pawn stops), `slideAliases` (the
  resting squares a tap may play), `slideEnd`, `fallen` — held to the
  engine's own board by `phase0/harness/test-ice-game.mjs` and the selftest.
  The cast rides variant.mjs (`iceIniKeys`, `spellPocket`), the record
  duel.mjs (`slide`, `castKind` `ice`, termination `pit`), the page
  main.mjs (the ❄ button on the portal's cast path, `moveAliases`, the
  slide chain, the log) and canvas-board (`setSlick`, `#iceTile`,
  `animateSlideChain`).
- `play/js/rays.mjs` — PORTALS ON THE GRID (2026-09-18; the body rule
  alone since Portals v4, 2026-09-19): the one ray walker (`walkRay` — a
  linked portal square ends every line, whatever stands on it, and nothing
  runs through a pair; a grid carries its pairs as `grid.portals`, set by
  `fenGrid` / `gridOf`, kept by `copyGrid`). tactics.mjs and threat.mjs
  walk it; Node gate `phase0/harness/test-portals-game.mjs`. (v2's
  `portalRoute` — the tunnel legs of a slide and an arrow's `via` — went
  with the tunnel.)
...
- `engine/` — the Phase 1.2.3 patch kit: `patches/dead-squares.patch` (the
  ...
  **`play/vendor/` carries the PATCHED pair** (deck + deck-search
  2026-09-26 — the ninth and tenth patches: the deck in the engine, the
  dig never pruned — on top of ice 2026-09-20 — the
  eighth patch: slippery squares, the slide, the pit `_`, the ice scroll —
  on top of: dead-squares 2026-08-26 +
  thread-stack 2026-08-27 + portals 2026-09-17 + wall-kinds + hammer
  2026-09-17 (one build, one gate) + portals-body + portals-cast
  2026-09-19 (the body rule alone since the tunnel's retirement, and the
  one-turn cast rebased on it — portals-v2 / portals-v3 until then), each
  behind a green rule-16 gate; the first two phone feel checks passed — thread-stack's on
  2026-09-01, on the v3 build; the portals pair was played the same day (the first portal duel), and so was the sledge pair (the first sledge duel on the phone: "Seems to work pretty good on mobile")). phase0's npm `node_modules` are
...
- `phase0/lib/` — shared infra: `png.mjs` (dependency-free PNG codec for the
  ...
  `fen.mjs` (largeboard FEN editing: walls `*`, hard walls `#`, furniture `^`, multi-digit runs, pockets),
...
node harness/test-portals-game.mjs   # PORTALS on the grid (play/js/rays.mjs, the body rule alone since v4): the walker on the engine gate's fixtures — a line ends at a portal square, nothing runs through a pair — the gods' attacks / landing guard / pins / terrain reach / exposure rule stopping at bodies; Node only
node harness/test-deck.mjs           # THE DECK (play/js/deck.mjs, 2026-09-25): the catalog against the scroll letters, the seeded shuffle, the opening hands as a pocket, the hand off the pocket, the draw, the meta cards, the mulligan, the enemy's deck by width, `?deck=`; Node only
node harness/test-cards.mjs          # THE CARD UI's pure half (play/js/cards.mjs, Phase 3.2): the catalog's face fields, the meta glyphs, the stamp's shadow, the fan's geometry, the gesture's tap / drag / hold, the drop preview's squares, the words; Node only
node harness/test-deck-duel.mjs      # THE DECK IN THE ENGINE, in the duel (the vendored pair in Node — overlay first): the opening hands in the start FEN, the engine's draw at the hand-over, the frozen pass and the link ply, the meta plays through an undo, the mulligan as the move @@@@, the win card ending the duel, no deck = the old controller
FFISH_JS=$PWD/../play/vendor/ffish.js node ../engine/tests/test-deck-ffish.cjs             # THE DECK, ffish half: the deck field, the slots, the draw, the redraw, the casts by kind, no card in check, a bared king with cards lost, the root by moves
ENGINE_JS=$PWD/../play/vendor/stockfish.js node ../engine/tests/test-deck-engine.cjs       # THE DECK, engine half: perft pinned to the native build, `d`, the win card on the spot, THE DIG at its measured depths, a 10x10 search with two decks
cd ../engine/forge && python3 native-test-deck.py <native-largeboard-build> --fixtures --random 500 --seed 21 --sweep && python3 horizon.py <native-largeboard-build>  # THE FORGE's native gate for the deck patch (the oracle grown for the deck) and the You Win horizon curve
node harness/deck-stress.mjs [--games 4] [--go "depth 8 movetime 300"] [--deck adept|ice,portal,…] [--enemy width|adept] [--width 3] [--gods off|restless] [--out results/deck-stress/x.jsonl]  # THE DECK's instrument: engine against engine on the canon controller with two decks — per card kind the cast count, the median ply, the share cast by the side behind, the cards dead in hand; per side the casts per 100 plies, the share of castable turns cast and (since 3.3a) the engine's REDRAWS; a cast's card is read off the caster's slot bindings in the FEN before it
node harness/test-ice-game.mjs       # THE ICE on the grid (play/js/ice.mjs): the slide's physics held to the VENDORED ffish's own board after every legal move on the engine gate's fixtures and on random ice boards (pits, pairs, patches, scattered ice, floors of it); the shove, the pit, the portal landing, the promotion, the aliases
FFISH_JS=$PWD/../play/vendor/ffish.js node ../engine/tests/test-ice-ffish.cjs           # THE ICE, ffish half: the fixtures, SAN, the field, push/pop along a slide, the fallen king, a check-flag sweep
ENGINE_JS=$PWD/../play/vendor/stockfish.js node ../engine/tests/test-ice-engine.cjs     # THE ICE, engine half: perft 1–3 pinned to the native build, `d`, the boards after the key moves, the search finding the shove into the pit
cd ../engine/forge && python3 native-test-ice.py <native-largeboard-build> --fixtures --random 800 --sweep --depth 4  # THE FORGE's native gate for the ice patch: the fixtures I1–I13, the oracle (oracle.py, grown for the ice) on random ice boards, xsweep, perft under a debug build's asserts
FFISH_JS=$PWD/../play/vendor/ffish.js node ../engine/tests/test-portals-ffish.cjs      # THE PORTAL SPELL, ffish half (v4: the body rule, no tunnel + the one-turn cast): the fixtures, SAN, the field, the strip rule, the check-flag sweep, the frozen ply / link ply / fizzle
ENGINE_JS=$PWD/../play/vendor/stockfish.js node ../engine/tests/test-portals-engine.cjs  # THE PORTAL SPELL, engine half (v4): perft on the fixtures over UCI (pinned to the native build), the casts through `d`, the strip at the root, the search, the one-turn cast
cd ../engine/forge && python3 native-test.py <native-largeboard-build> --fixtures --random 800 --sweep --depth 4  # THE FORGE's native gate for the portal patches (the body rule + the one-turn cast): hand-derived fixtures, the independent Python oracle (oracle.py) on random positions with pairs, scrolls, halves and passes, the scratch tree's xsweep, perft under a debug build's asserts
FFISH_JS=$PWD/../play/vendor/ffish.js node ../engine/tests/test-hammer-ffish.cjs        # THE SLEDGEHAMMER + THE HARD WALL, ffish half: `#` parsed and round-tripped, K*f2, never in check, a pinned rook hammers, `#` ≡ `*` without the key, per-colour keys, perft 1–4, a check-flag sweep
ENGINE_JS=$PWD/../play/vendor/stockfish.js node ../engine/tests/test-hammer-engine.cjs  # THE SLEDGEHAMMER + THE HARD WALL, engine half: the same fixtures over UCI, `d`, a 10×10 search with both kings hammering
...
15. **The vendored pair is a MATCHED SET built from one patch, two trees,
    two toolchains — and TEN patches now: dead-squares, thread-stack,
    portals, wall-kinds, hammer, portals-body, portals-cast, ice, deck,
    deck-search, applied in that order (portals-body is portals-v2's body
    rule with the tunnel half removed and portals-cast is portals-v3
    rebased, since the tunnel's retirement 2026-09-19; ice is the slide,
    the pit `_` and the ice scroll, 2026-09-20; deck is the deck in the
    engine and deck-search the never-pruned dig, 2026-09-26 — their source
    is `engine/forge/apply-deck.py` + `apply-deck-search.py`).** ffish comes from FSF mainline (`src/Makefile_js`,
