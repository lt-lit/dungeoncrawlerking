...
// page its played card flies to the board and holds for a beat.
{
  ...
  expect(!fan.rowHidden && fan.lay.length === 4 && fan.lay.map((c) => c.kind).join(',') === 'reveal,undo,ice,portal', `the fan holds the four cards of the hand, in the slots' order — the draw order at the start (THE DECK IN THE ENGINE) (${fan.lay.map((c) => c.kind).join(' ')})`);
  ...
  expect(fan.arts.every((a) => (a.w === 12 || a.w === 11) && a.cssW === a.w * 4 && a.pip) && fan.arts.map((a) => a.name).join(',') === 'Reveal,Undo,Ice,Portal' && fan.arts.every((a) => a.text.length > 0), `every face carries its art at 4× (${fan.arts.map((a) => `${a.w}→${a.cssW}`).join(' ')} px — the portal's ring is 10 wide, the rest 11, plus the shadow), a cost pip, its name and one line (${fan.arts.map((a) => a.text).join(' · ')})`);
  expect(/\bspell\b/.test(fan.arts[2].cls) && /\bplayable\b/.test(fan.arts[2].cls) && /\bmeta\b/.test(fan.arts[1].cls) && /\bdim\b/.test(fan.arts[1].cls), `a spell card is rimmed as playable, the Undo card dimmed at ply 0 (${fan.arts[2].cls} · ${fan.arts[1].cls})`);
