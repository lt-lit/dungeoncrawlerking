...
// (the debris image's deterministic PNG) and the ruin tiles' chip strip. No DOM, no
...
// --- the PNG (the debris image's bytes) ---------------------------------------
{
...
// ---- THE ATLAS'S PROP BOXES AND THE EDGE-ON LEAF (2026-09-11): a prop role
// is a 16×32 box on every theme and nothing else is — the edge-on leaf is a
// 16×16 sprite (the designer's 5×16 profile door in the band's middle) and
// the classic row's crate, barrel and chest are 16×16 drawings; reporting
// the box for a 16-tall tile floats it a square north, which the pixel
...
// every theme's has the file's exact shape.
{
  ...
  expect(['door', 'door-edge', 'doorway'].every((r) => atlas.tileOf('hall', r).h === T), 'the face-on leaf, the edge-on leaf and the doorway posts are 16×16');
  expect(['crate', 'chest', 'barrel', 'wreckage', 'door-edge'].every((r) => atlas.classicTile(r).h === T), 'the classic row is 16 tall throughout, its edge-on leaf included');
  expect(atlas.tileOf('hall', 'door-edge', { doors: 'crypt' }).theme === 'crypt' && atlas.tileOf('hall', 'doorway', { doors: 'crypt' }).theme === 'hall', 'the door set swaps the edge-on leaf and leaves the posts to the theme');
  ...
  const shapeOk = Object.keys(index.themes).every((theme) => {
    ...
    for (let y = 0; y < T; y++) for (let x = 0; x < T; x++) {
      const inFile = x >= X0 && x < X0 + file.width;
      const want = inFile ? file.data[(y * file.width + x - X0) * 4 + 3] : 0;
      if (png.data[o(x, y) + 3] !== want) return false;
    }
    return true;
  });
  expect(shapeOk, `every edge-on leaf has the file's exact shape at columns ${X0}–${X0 + 4} (hall, castle, crypt, classic)`);
  const o = at('hall');
  let same = true;
