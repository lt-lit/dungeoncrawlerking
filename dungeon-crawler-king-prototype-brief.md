...
- **Gap math** `[2026-09-08, the barrier by hand: the barrier drops at gap EXACTLY 4 (empty ranks between the two armies' closest rows), the kings aligned on one file, the depth computed from the molding — a 2-deep kit against a 2-deep enemy is 8 ranks, the kings 7 apart, and deeper molded armies use the 9- and 10-rank variants by construction; the "arenas may not use them" clause below is retired (the wave-6 bed is 10×10). The band for the real trigger is still the conversation §10 defers.]`: the trigger condition enforces **gap ∈ [2, 4]**.
