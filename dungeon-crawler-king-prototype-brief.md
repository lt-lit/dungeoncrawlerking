...
- **Phase 2 — Exploration slice** `[NEXT — designer 2026-09-07: "I've been putting off Phase 2 long enough"]`. **It opens with the rendering pipeline (§2 item 5):** one full-board buffer at native 16-px-per-tile resolution, every layer written into it by the existing pure painters, scaled to the screen once at an integer device-pixel factor (the board no longer fills the viewport exactly and steps in size — the pixel-art norm), with a camera for the 100×100 floor. Decided after the tile-grid piece work of 2026-09-07 showed the DOM board's cost: every layer is a separate image the browser resamples at a fractional scale, so alignment is a property each layer must re-earn by construction (cell-sized boxes, measured row rectangles, position baked into tiles, a two-browser gate), the debris image sits a sub-device-pixel off the floor, slides shimmer off-grid, and a DOM grid of ten thousand cells cannot carry the overworld. The spike comes first: a full-board canvas at integer scale running the existing painters, judged for flicker on the designer's Firefox/Windows and Android Firefox before anything is built on it (the "no canvas on the board" rule was written against PER-CELL canvases mixed into DOM compositing — one board-sized surface is a different animal, but the phone decides). What carries over: the pure modules (`piecetiers`, the debris ledger + painter, `classifyTerrain`, the autotile masks, the atlas and the repack tool — one atlas PNG instead of data URIs), the rules and the Director. What is rewritten: board-ui's DOM painting, style.css's tile rules, the arrow/mark overlays, the FLIP slides and quake fx, the smoke suites' renderer checks; the replay analyzer shares the new renderer. Then the slice itself: one hand-built map, two summoners with different levels. Army-as-avatar movement (blob + marching pattern, visible enemy armies), LOS/hunt/pursuit state machine, threat display, trigger → barrier → FEN pipeline proven end to end.
  ...
  - **Milestone 5 — THE GENERATOR `[NEXT — decided 2026-09-08, the trigger conversation]`.** The conversation opened as planned and turned on the ground: the designer ruled the arena ALWAYS 10×10 with a minimum gap of 2, no hallways under 3 wide, and — on the measurement that no 10×10 crop of either hand-built map comes near the wave-6 bed — both maps retired and a generator with several algorithms and per-floor styles made the milestone ("randomized dungeons are a requirement"). The plan, its lints and its five starting styles are in CLAUDE.md § Phase 2, "The trigger conversation"; the first PR is the lints, the prefab-grid skeleton that lays the 36 arenas as pieces, and the gallery.
