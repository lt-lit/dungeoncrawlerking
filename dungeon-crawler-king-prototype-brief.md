...
5. **The 16×16 tile grid is the picture** `[NEW 2026-09-07 — designer: "I want to commit to 16x16"]`. Every drawn thing — floor, walls, props, debris, pieces, particles, marks — is 16×16-pixel art on one native-resolution grid, composed in ONE buffer and scaled to the screen ONCE, by a whole number where the screen allows it. No layer is resampled on its own, no sprite has a size or an alignment of its own, and nothing is positioned in fractions of a tile pixel. Phase 2 opens with the renderer that makes this true (§10); the Phase 1 DOM board carries the duel until it lands, at the price of the alignment machinery documented in `play/README.md` § "Art themes" `[2026-09-07, the next session: it landed — the canvas board is judged fine on desktop and phone, integer scaling included, and the DOM board is RETIRED the same day (§10 milestone 2); the atlas is the one art source, CLAUDE.md rule 19]`. `[2026-09-07, designer: integer scaling is the default and preferred, but "not a dealbreaker" — a FILL fallback (the exact quotient, one nearest-neighbour resample, uneven pixel widths but every layer still aligned) is allowed where the integer step is too small for the screen or the thumbs. The scale table for current phones (a 10-file arena lands between 5.0 and 7.1 mm per tile everywhere, within a step of the DOM board) is in CLAUDE.md § Phase 2.]`
...
- **Width = local room width**, up to FSF's max files. Side barriers are only added when needed to fit FSF limits. Board caps: 12 files × 10 ranks (largeboard build). `[2026-09-08, designer: THE ARENA CAP IS 10×10 — the engine's 12 files are the engine's, not the game's; a 12-wide board is k 5 on a phone, "officially too small for my thumbs". The barrier (§10 Phase 2 milestone 4c) caps the room's width at 10.]` `[2026-09-08, THE TRIGGER CONVERSATION — designer: "just make it always a 10x10 arena": THE ARENA IS ALWAYS 10×10, a fixed box on the player's king with the room's walls inside it as terrain; width is no longer measured, and the kings are always 9 apart. CLAUDE.md § Phase 2, "The trigger conversation", carries the record.]` `[2026-09-09: THE BOX IS CENTRED ON THE KING — four files to his left, five to his right, the room's walls where they fall. The first cut slid it to keep the room's floor inside, and the designer's first log had the army hugging the arena's edge: "why is the arena bounds not centered around the armies?"]`
...
- **The player's opening kit is a 3×2 army — K+R+N + 3 pawns** `[PROVISIONAL in composition]` `[2026-09-10, THE ENEMIES SESSION — designer: THE KIT IS 4×2: K + R + N + B and FOUR pawns (value 15; the molder lays it N K R B over P P P P, the king second from the left because a width of four has no middle). ENEMIES ARE 3 WIDE FOR NOW — a king, two pieces and three pawns — the two pieces drawn from the run's seed in a NINE-TO-THIRTEEN band (knights and bishops up to two rooks; no queens at width 3), so the first floors carry the §7 player-favoured edge and the level telegraph reads in the composition. `army.mjs OPENING_KIT` is the one constant every reader imports.]` — and the army GROWS through the run (3×2 → … → 8×2), terminating at the width cap (§8). Keep a major piece in the kit: rook mates work at any arena width.
...
- **Initiative:** whichever side's move *completes* the legal duel condition plays White. Ambusher moves first; a player who deliberately steps into alignment holds White. `[2026-09-08: "plays White" reads "moves first" — the player ALWAYS holds White (his king's rank is White's home edge under the king-anchor rule, §4.2); initiative is the deal's turn field, never a seat swap. The barrier by hand carries it as a toggle.]`
...
- **Gap math** `[2026-09-08, the barrier by hand: the barrier drops at gap EXACTLY 4 (empty ranks between the two armies' closest rows), the kings aligned on one file, the depth computed from the molding — a 2-deep kit against a 2-deep enemy is 8 ranks, the kings 7 apart, and deeper molded armies use the 9- and 10-rank variants by construction; the "arenas may not use them" clause below is retired (the wave-6 bed is 10×10). The band for the real trigger is still the conversation §10 defers.]` `[2026-09-08, the trigger conversation: THE GAP IS AN OUTPUT — ten ranks minus the two moldings, the kings always 9 apart, gap 6 on open ground (the wave-6 spacing) — with a MINIMUM OF 2 (designer). There is no band. A gap scaled to the player's width was proposed and superseded within the hour by the fixed box (§4.1, §5.3).]`: the trigger condition enforces **gap ∈ [2, 4]**.
 Gap 2 = ambush-sharp (the puzzle sweet spot), 3–4 = standard (4 = classic chess spacing). `[REVISED: gaps 5–6 produced 100+-ply grinds at every width in the Phase 0 smoke sweep — the ranged/rider band is cut; the catalog keeps 9/10-rank variants but arenas and the trigger may not use them.]` `[UNDER RE-INVESTIGATION 2026-08: molded armies are no longer fixed 2-deep, so gap and formation depth decouple; the proving-grounds lab tests gaps 1–6 and whether the ideal gap scales with army size — designer expects a practical trigger band of 2–5. The lint floor for a legal deal is gap ≥ 1.]`
...
2. **A severity ladder, not one move.** Restlessness buys escalation, and the
   ...
   **breach** (`^` → floor, the line opens for real), **displace** (ONE piece,
   ...
   to the closer, and NEVER on a piece — quakes cannot swallow,
   `[designer-final 2026-09-01]`). A quake SPENDS AN ACTION BUDGET, drawn rather than computed,
   ...
   0/7073), they cannot hand out material by MOVING it (the whole arena03
   free-rook class) `[AMENDED 2026-09-01: a breach can hand it out by
   EXPOSING it — live play opened a line onto a queen, so the promoted
   no-new-winning-capture guard (threat.mjs `editExposes`) now prices every
   line-editing rung; weaken alone stays safe by construction]`,
...
4. **The stage stays the stage `[NEW 2026-09-01 — the conservation brake]`.**
   The Director freezes the AUTHORED terrain census at duel start and damps
   both terrain rungs as standing terrain (walls + crates; holes never count)
   falls toward stripped — full weight above `conserveAt` (0.6 of authored),
   silent at `conserveFloor` (0.3). It counts the BOARD, so armies eating
   crates draws the brake down exactly like breaches do. A god-crate ledger
   (kept like `holes` — the FEN cannot tell authored furniture from god-made)
   biases breach targeting +3 toward crates the gods themselves minted: they
   eat their own rubble before the designer's furniture. Still structural, no
   eval, replay re-derives the anchor from the start FEN. Measured (A/B,
   `results/godlab/brake-ab-findings.md`): calm's floorplan terrain
   remaining 18%→32%, wrathful 5%→17%, pacing and the alarm metric
   unchanged, displacement share up as the fall-through absorbs braked
   actions.
...
1. **Displacement** — a piece slides to an adjacent empty square. Under v2 this was the primary tool and the *only* mechanic that could reopen a locked position — measured: a crumble frees a terrain-locked pawn **0 times in 7,073** instances (it cannot — the block *is* a wall and a crumble only adds walls); displacing that pawn works 12.7% of the time. Mean effect on total legal moves: displacement **+0.08**, crumble **−1.55**. That finding is exactly why v3 added weaken/breach: displacement unlocks by moving the PIECE, terrain edits unlock by removing the WALL, and the 12.7% success rate was displacement straining at a job it is structurally bad at.
   - **Symmetric-preferred pairing `[REPEALED v3 2026-08-31]`.** v2 moved one piece **per side** per quake, because one-sided stirs hand whole games away (measured: the median one-sided eval flip was a *mate-score transition*). Repealed on two grounds: exactly one piece of each colour moving on every quake is a signature no player misses, and pairing was symmetric in COUNT — the two findings below establish that count was never the protection; the SEE landing guard is. v3 displaces ONE piece, either side, best tier over both, and the one-sided catastrophe is answered by the guard plus the fact that displacement is now one rung of four and the other three cannot favour a side at all. The findings that survive it:
     ...
     - `[UPDATED 2026-09-01]` Discovered attacks from the vacated square: **closed** — `threat.mjs editExposes` (the promoted no-new-winning-capture rule) prices what every line-editing action uncovers or severs. The crumble value-guard question: **mooted** — quakes cannot swallow pieces at all (see the crumble rung above), so there is no victim to price. Still open: *rescuing* an already-hanging piece (also a gift), and pinned pieces counted as defenders.
...
2. **Crumble** — a square collapses into a permanent HOLE. **It never takes an occupant with it `[designer-final 2026-09-01 — was "taking any occupant with it"]`: quakes cannot swallow pieces.** Occupied squares are simply not in the crumble vocabulary (a hole opens in bare floor or not at all — a wrathful hole ate a knight at ply 13 in play, which is not a closer, it is a mugging), which subsumes the old never-strip-a-last-piece guard and moots the value-guard question below. `[v3]` Demoted from a mid-game event to the closer, kept landing by the debt cap (which now counts every rung), and at most ONE per quake. Crumbles are still **the clock**: holes are the only monotonic force in the system and the whole termination guarantee rests on them (see "Holes are forever" above).
...
### 4.9 The ice spell — slippery squares, the slide and the pit `_` `[BUILT 2026-09-20 — designer-specified rules; engine substrate is engine/patches/ice.patch, the eighth patch; one scroll a side for the stress test; PLAYED on the phone 2026-09-24 — in]`

**The ability `[designer, 2026-09-20 — "Time to implement a spell that creates a 3x3 ice patch"]`:** a cast turns the floor of a 3×3 patch SLIPPERY for the rest of the duel. Oil and ice will share the slip logic (one physics; oil's difference deferred). Momentum transfers: "when a sliding piece bumps into another piece on the ice, it should transfer the momentum." One cast per side per game for now.

**The rules `[designer, the same day — exactly, no special cases]`:**

- **The stop rule:** "a sliding piece should keep sliding until it's not on a slippery surface anymore. Or until it hits a wall or another piece." A piece whose move ENDS on a slippery square slides on in the direction it moved and comes to rest on the first square that is not slippery; a wall, a crate, the board's edge or a piece stops it on the square before. Only the square a move ENTERS matters — a rider's line runs over ice without stopping.
- **The pit:** "Sliding pieces can fall into holes and die. So I guess we do in fact need to differentiate between holes and indestructible walls." A hole is its own glyph to the engine now, `_` — a square nothing enters by a move (as `#`) but that a sliding piece falls into and is gone from; a crumble writes `_`; bedrock and the map's edge stay `#`. §4.8's "one glyph for a pit and a bedrock wall" is amended by this.
- **Kings:** "either king can slide and fall into a hole, just make a move illegal if ends with your king dead or in check, just like always." A move that drops your own king into a pit is illegal; a shove that drops the enemy's wins on the spot.
- **Momentum (Newton's cradle):** a slider that bumps into a piece STANDING ON ICE stops where it is and the piece it hit slides on the same way, passing it on in turn — either colour, kings included, a chain as long as the ice. A slide never captures; a piece standing on plain floor is an obstacle like a wall.
- **Portals:** a portal square is never slippery; a slide that reaches an EMPTY portal square is a landing (out of the twin, swapping with whatever stands there, at rest); a piece on a portal square is an obstacle.
- **Shapes:** "don't assume that every slippery patch is always going to be a 3x3 placed spell" — more ice spells of various sizes, whole floors that are mostly ice. The engine keeps a bitboard of slippery squares of ANY shape (`~e5` entries in the FEN's trailing field); the 3×3 cast is one writer of it.
- **The rest, agreed:** a knight lands where it jumps (no direction, no slide); a capture on ice slides the captor on, en passant included; a pawn that STOPS on its promotion zone promotes — its own move to the piece named, a shoved pawn to the strongest; the cast is a one-ply drop of the scroll, never in check, on any non-terrain square of the board's two MIDDLE rows (ranks 5–6 on the box), occupied or not, so a patch never comes nearer than two rows to a king row (the portal's margin, §4.7); the gods leave slippery squares alone (no landing on one, no pit under one); the ice persists for the duel; nothing on the walk.

**The engine shape** (`engine/patches/ice.patch`, applied eighth; `engine/README.md` § "The ice patch"): a SLIDE move type encoded on the entered square, the physics (`slide_outcome`) recording every relocation in the state so undo is the state pointer; `slide_attacks_king` judges legality and check on the virtual board; a rider's quiet move onto a run of ice is generated ONCE, at the last empty ice square before what stops it; evasions generated whole and filtered; the ice scroll a custom immobile piece (`i`, `iceScroll`, its cast rows the piece's mobility region), never counted by the strip rule; `slick_effective` = the slippery squares less the portal squares; a side whose king fell has lost (`is_immediate_game_end`). Validated natively against the independent Python oracle (55 fixtures, 4,200 random positions with ice of every shape, pits, portals and scrolls, the sweep, perft 4 under asserts), ice-free boards node-identical to the seven-patch build; the rule-16 gate green on both binaries.
...
**The glyphs `[designer, 2026-09-21]`:** "On move hints, there's just a square outline for both portal and ice. How am I supposed to know what spell it's suggesting?" — built the same day: every PROPOSED cast (a hint, a cast in the analyzer's lines — the board does not show the spell yet) is its square framed at its edge with its spell's glyph inside — the portal's ring, the ice's snowflake — in the arrow's colour, and the hint list wears the same glyph before the move's SAN (the hammer glyph's method, §4.8); a PLAYED cast (the enemy's own, the analyzer's ply) keeps the bare frame, since the board shows the spell itself there and a glyph over a pair's ring would hide whose it is. The second cut, the same day, on the designer's verdict ("outlines around all 9 tiles is way too loud. Just the one center square is fine. Also the icon could use some work" — with a reference snowflake): the centre square alone framed, the snowflake redrawn to the reference (six arms, forked tips, a hollow hub, 11×11), a drop shadow in place of the halo that had filled the flake into a black tile.

**Played `[designer, 2026-09-24, the phone, with the log]`:** "Looks like it works as far as I can tell" — the ice and the spell glyphs are in. The log is the fourth committed sample, the first ice duel (`replay/samples/`, `?sample=4`): both sides cast the ice and the patches overlapped, a pawn's double step onto the patch slid three squares on, a rook was stopped on the patch by a pawn beyond it, the enemy cast its portal pair after the ice, and both kings hammered.

### 4.10 Spells as cards — THE DECK `[NEW 2026-09-25 — designer-specified; STAGE 1 BUILT the same day (play/js/deck.mjs, no engine change); the later stages are the agreed plan]`

**The idea `[designer, 2026-09-25]`:** "Could we have spells as cards drawn from a deck? I know FSF doesn't work with random elements and hidden information, so what if we just made both decks and hands fully visible to both players." The answer on record: a shuffled deck both sides can read is neither randomness nor hidden information to the engine — the shuffle is one seeded draw at the deal, like the floor and the army, and from then on a fixed sequence; THE HAND IS THE POCKET the engine already searches; the one thing it does not do natively is draw. The deck is the spine of the RUN, not of the duel: it is the container every spell was promised as "an upgrade later", the reward the walk had none of, and the axis on which enemies differ beyond width. §12's v1 non-goal "no hands/pockets as a core economy" is REVERSED by this section (it predated every spell; the pocket became the spell system's home the day portals shipped).

**The rulings `[designer, the same session]`:**

- **A hand of FOUR.** "Draw up to that hand size at the start of each turn. Perhaps with an option to spend your turn discarding any cards you don't like and drawing a new hand." Two decks. Starter decks once enough spells exist; cards earned in duels, found in chests, from dungeon puzzles; "shuffle and draw a new hand at the start of a duel" (the deck is the run's collection — spent cards come back for the next duel); deck strength scales roughly with army size.
- **NO ARTIFICIAL VALUES — a standing rule.** "When the engine chooses to play a card, it should do so because it's objectively the best move, not because of some arbitrary number we picked. The goal is always for the engine to play as optimally as possible. If we don't like how often the engine plays spells, the answer is to change how the spells work. DO NOT artificially value spells just to make the engine play suboptimally." A scroll's piece value stays 0 — a scroll in hand has no material worth, so the search alone judges a cast — and the cast rate is tuned by a spell's RULES (its range, its area, its cost in tempo, its copies in a deck), never by a number. §12's "no engine handicapping" applies to cards in full. (A per-card value dial was proposed and withdrawn the same session; the +150 / −100 self-play runs stand only as evidence that an optimizer answers its inputs.)
- **A cast is the caster's move.** Free casts (cast, then move) are buildable on the portal cast's frozen-pass machinery (cast, the opponent's forced pass, the caster's move — alternation kept, not the true double move §12 rules out) and are NOT the rule: a free card competes with nothing and is played the turn it has any value, a spell every turn while cards last, at the cost of a turn's tree being casts × moves. A QUICK tier for minor cards with few targets, one a turn, may come later.
- **Meta cards.** Reveal (the engine's best three lines for this turn) and Undo (take back your last move and the enemy's reply): the player's alone, they cost no move and never reach the engine — they live beside the pocket, not in it. The enemy's deck holds only cards the engine can search; an engine reveals nothing to itself and never undoes.
- **Lethal by default, varying by card.** "Shocks and burns should be lethal. May vary by card. Might have lesser electric spells that stun pieces for a few turns in an area. Or ice spells that turn an enemy piece into an ice block until it can be freed by a destructive effect." A burn or a shock is a capture (restored on a win, §8); a cast that kills your own king at once is illegal, one that kills the enemy king wins on the spot; fire kills at its tick and never forces a reply. Stun: cannot move for n turns, still attacks, defends and can be captured (a pinned piece's reading). Entombed: inert — no moves, no attacks, uncapturable, a wall to lines — freed by any hit from either side, melted by fire into water; kings never targets.
- **Destruction as HITS, with areas.** A square takes a hit: an intact breakable wall becomes cracked, a cracked wall or crate becomes floor, bedrock ignores it, pieces are untouched; an area card hits every square of its shape (a 3×3, a line, a cross) — "spells that can both crack a wall and destroy an already cracked wall" is one card cast twice or two hits. A cracked wall from a card joins the one crate ledger (§4.8).
- **Elements as MATERIALS.** Every square carries a material (stone, ice, water, oil, fire); element cards are a transition table on the material they land on — fire melts ice to water, ignites oil, burns wood, fizzles on water and stone; water puts out fire and stops a slide; ice freezes water and puts out fire; lightning shocks a whole connected body of water and ignites oil; oil is slippery like ice (one slide logic) and is fuel. FIRE SPREADS "in a deterministic predictable intuitive way": once per full turn, to orthogonally adjacent squares only, onto fuel only; oil burns one turn, wood two, a burnt square is scorched stone; stone, water, pits and bedrock are firebreaks. The tick runs inside the engine's move, so the search sees every step of the burn inside its horizon and the board can tint the squares that burn next. Barrels become explosive furniture (a hit on their 3×3 when fire reaches them).
- **The gods.** "If we give the players enough agency and options for messing with the arena, we may be able to largely retire the gods." Cards can retire the gods' STIRRING (the ladder's weaken / breach / displace on a seeded pick — what the terrain family hands to the players, readable where a quake was unreadable by design) and cannot retire their CLOSING (no duel runs forever: a player who wants to stall casts nothing, and the engine does not see through a deep fortress either — gods-off never terminated on 42% of the old bed). The plan: keep the dead-board backstop and the terminal crumble, move the ladder to a chaos preset, and measure the closer's firing rate in the god lab with decks before anything is retired.

**Measured before building (the self-play instrument, engine vs engine on the vendored pair, gods off, 2026-09-24/25):** at value 0 the engine casts on about one castable turn in twenty and every cast scores within a pawn of its best plain move (most within a quarter pawn) — it casts when a spell is worth about a tempo; the side BEHIND does most of the casting and does it early (the enemy is usually the side behind: in the phone logs it opened with a portal in two of three duels); situational cards (the ice on empty middle rows) are held for twenty turns and cast when a slide tactic appears; casts are listed once per card KIND in hand, not per copy (a fat hand's root was the same 68 casts as the shipped hand's); 68 casts at the root cost about 1.5× the time to reach depth 16 and about two plies at the live ten seconds — HAND SIZE IN KINDS is the engine's cost, and range per card is the rule-shaped lever, never a value.

**Stage 1 — BUILT 2026-09-25 (no forge):** `play/js/deck.mjs` — the catalog (ice, portal, Reveal, Undo), the Adept starter (3 portals, 3 ice, Reveal, Undo), the enemy's deck of width − 1 spells drawn by seed, the seeded shuffle off the deal's seed, THE POCKET IS THE TRUTH for spells (a card's scrolls ARE the holdings; a half-spent portal is still a card; the meta cards sit beside the pocket in the deck state), the draw to hand size, the mulligan, the record's shape; `duel.mjs` — the refill for the side to move at each turn's start as a bare position for the engine (never on the frozen side's pass or the link ply), `deck` on every state and `metaPlays` on the record, the mulligan as a `--` ply the gods read as cold, the piles rewinding with an undo; the deals take the opening hands' `pocket`; the run carries `deck` (`dck-run/6`); the log carries `decks` and `metaPlays`; the page — Options → Spells → Deck (the starter, or "everything, no deck" for the stress-test set), `?deck=off|adept|<kind,kind,…>` (a list deals in its own order), the spell buttons counting cards, Reveal / Undo / Redraw buttons, the pile under the bar, the enemy's hand and pile in its bar, Reveal running the hint probe for the turn without Cheater Mode, the Undo card taking a turn back and staying spent, the redraw spending the turn; the walk's run dealt from its deck, the enemy's by its width from its own seed. Gates: test-deck, test-deck-duel (the vendored pair in Node), ui-smoke's DECK block, selftest.

**The staging (agreed 2026-09-25 — "possibly a bit ambitious to build all in one go… we want to get a healthy variety of spells before we really even attempt to nail down the balancing"; NUMBERED the same day as Phase 3's sub-phases in §10 — 3.0 the record, 3.1 the deck, 3.2 the card UI, 3.3 terrain edits, 3.4 timers and piece states, 3.5 materials, 3.6 the run's economy, 3.7 balance and the gods' preset, 3.8 floors, 3.9 piece identity):** stage 0 the record (this); 1 the deck (built); 2 TERRAIN EDITS, the first forge — hits and shapes (Crack, Smash, Demolish, Blast, Lance), Sink, Raise, Harden, the sledge as an ENCHANTMENT (a per-colour flag a cast on the king's square switches on, retiring the always-on setting), a per-type drop region for Reinforcement, Golem by ini; 3 TIMERS + PIECE STATES — Stun, Entomb, Ward, melting ice, Palisade; 4a MATERIALS without fire — Water, Oil, Lightning; 4b FIRE — the tick, lethality, scorch, barrels; 5 Gust on the slide trigger, cards from chests and loot, deck strength by width; later PIECE IDENTITY (the twin: a duplicate adjacent to the original, both dying together — every sub-problem has a precedent, atomic chess's multi-removal and the portals' virtual-board legality, but it needs a tag that follows a piece through every relocation, a mechanism nothing has needed yet). Balance is RULES only — copies, range, area, hand size, enemy deck size, lethal or lesser — set after 4b with the library at twenty-plus kinds, read by a deck-stress harness and the god lab with decks. The deck's shape (four, draw-to-four, the mulligan, lethal by default) is not deferred: every card is authored against it.

**Open, the designer's:** whether a hit frees an ice block in one, or a block takes two like a wall; whether the lethal fire cast is its own card or the ignite card with a body count; which four transitions are in the first table; and, from the first deck duel's log (2026-09-25, `?sample=5` — the designer: "The enemy portal placement is pretty weird. What's up with that?"), whether THE PORTAL NEEDS A SPREAD RULE: the enemy, losing from ply 1, cast three portal pairs as line-blocking BODIES (a3–d3 in the player's camp, an adjacent g5–g6 beside the ice, j8–d4 six plies from mate) — the no-values rule working as written, a losing engine casting the least-bad move its eval finds; if the pairs should read as transport, the fix is a rule of the spell (a minimum distance between a pair's two squares, in the link's drop generator — a forge; or a cast kept out of the opponent's half), never a value.

**The card UI — Phase 3.2 `[PROPOSED 2026-09-25 — designer: "These little buttons aren't gonna cut it. I wanna take notes from other card wielding games like mtg arena and hearthstone. Probably cast spells by dragging them from the hand to the field, that kind of thing" — and ✅ BUILT the same day on the designer's "Looks good. Agree with all 4 calls" (the area preview under the finger, the enemy's played-card beat, the redraw as a deck-tap with a confirm, the portal's second square by a tap with cast mode held or the card dragged again); the record is `play/README.md` § "The card UI"]`.** What transfers from Hearthstone and MTG Arena on a phone, and what does not:

- **The hand is a fan.** Four cards under the board where the spell buttons stand, overlapping and tilted, every copy its own card (no ×n), the playable ones rimmed (Hearthstone's green glow — here the spell blue and the meta gold), the unplayable dimmed: not your turn, no legal target, nothing to take back. A card face is a frame in CSS around pixel art off the atlas' own glyphs (rule 19: the portal's ring and the ice's snowflake from pixelarrow, at three or four times), the name, one line of rules text from the catalog, and a COST PIP in the corner — a filled "your move" pip on a spell, a dash on a meta card. No mana.
- **Drag to cast.** Press a card and it lifts and grows; over the board the legal targets light (the cast mode's own squares), the square under the finger is framed and an AREA card's whole shape is tinted under it; release on a lit square casts through the existing cast path; release anywhere else and the card snaps back. The ghost is a fixed-position DOM card and the drop test is the board's `squareAtPoint` — nothing new must align with the floor, so rule 18 is untouched. The shape under the finger is a DRAG preview that vanishes on release, not a hint, so the ice hint's "way too loud" ruling does not reach it — proposed, the designer's to confirm.
- **Tap to cast stays.** One gesture path split at eight pixels of travel: a tap lifts the card into cast mode (the lit squares, then a tap on the board), a drag carries it. A thumb that hates dragging still plays, and the smokes keep their tap path.
- **Long-press to read.** A raised card with its full text; a hover on the desktop.
- **The enemy's hand, face-up.** Mini cards along its bar (visible decks are the ruling; there are no card backs), its deck a stack with a count. When the enemy casts, its card flies to the board and holds enlarged for a beat before the spell lands — Hearthstone's opponent-played-card reveal, the legibility beat this game keeps asking for.
- **The draw.** At the turn's start the drawn card slides from the deck stack into the fan, with the log line it has today.
- **The piles.** The deck a stack with its count, a tap opens the pile in order; the spent pile beside it. THE REDRAW is a tap on the deck stack that opens a sheet — "discard your hand and draw four; this spends your turn" — with a confirm, so a turn is never spent by a slip.
- **The portal's two squares.** Drop the card on the first square (the half opens, the enemy's pass plays), then the link is a tap on a lit square with cast mode held, as today; a drag from the first portal to the second is the alternative to try on the phone.
- **Sizes.** On a 390-px phone: four cards of about 84×126 in a fan under the board, the piles at the right, the hint list and the gods line under the fan; the wide layout grows the cards under the board column. The board keeps its height budget — the fan replaces the button row and the deck line, it does not sit over the board.
- **Gates.** ui-smoke drives the drag through pointer events and keeps the tap path; the two mock frames (the resting hand, a card dragged over e5 with its 3×3 lit on the real board) went to the designer with the proposal.
- **As built (2026-09-25):** `play/js/cards.mjs` (the fan's geometry, the art — a spell card's art IS its hint's glyph, the meta cards an eye and a counter-clockwise arrow — the gesture split at 8 px of travel and 450 ms, the drop preview's squares, the words; `test-cards.mjs` 54) + main.mjs § THE CARD UI (the fan, the ghost with its tooltip, the board's new `hover` / `area` marks, the reader, the deck sheet with Redraw → Confirm, the enemy's minis and mini stack, the played-card beat awaited before the spell paints, the draw from the stack); the buttons and the deck line are gone; ui-smoke's CARD UI block drives the drag through Playwright's mouse (438 ok / 0 failed). THE VERDICT, the same day (designer, with a log — the fifth committed sample, the first deck duel): "Pretty funky but it works. Looking forward to more card variety." — IN.
...
- Turn-based, tile grid. **The army IS the avatar** `[REVISED — the army-avatar pivot]`: the player moves their whole formation as ONE unit — a flexible blob that holds the customized marching pattern where the ground allows and deforms through narrow gaps and around holes. `[PROVISIONAL: 1 tile/turn, 8-directional — speed parity with hunters assumed but not settled]`
  - **THE ONE MOVEMENT RULE `[NEW 2026-09-07 — designer's d-pad / rotation / pathfind-home notes, folded into one rule; PROVISIONAL]`.** A virtual d-pad (WASD on a keyboard) moves the formation's ANCHOR — the king — one king step; two buttons turn the marching pattern a quarter turn around the king. Every turn, every other piece takes the single move — a king step OR its own chess move on the world grid (designer: "so pawns can make adjustments and knights don't have to do an awkward dance") — that most reduces its path distance (BFS on the map, never Chebyshev — walls) to its slot in the pattern. Under that one rule an army on open floor moves in unison (each piece's best step IS the d-pad step), a wall in front of one piece makes the blob flow around it, a straggler walks home (a rook slides home in one, a knight hops, a bishop on the wrong colour walks, a pawn that cannot move backward as a pawn king-steps), and a rotation makes the army about-face over a few turns instead of teleporting. Conflicts resolve king first, then nearest-to-slot, two passes so a piece can step into a square a comrade is leaving. **Individual chess moves** (tap a piece, its moves on the world grid — sliders blocked by walls and pieces, pawns forward-only along the army's facing — tap a target) spend the turn on ONE piece; smashing furniture is a capture. **Automatic moves never capture** (a returning rook stops short of a crate — the army never auto-smashes; only a deliberate move does). A **wait** passes a turn in place so stragglers reform without moving the anchor. Rotation is a free re-target (the pieces move on the next turn) `[OPEN: or a turn]` `[RESOLVED 2026-09-08, milestone 4b: A ROTATION COSTS A MOVE — a wait with a turned pattern]`. The king never makes an individual move `[proposal]` `[built so in 4b]`.
 **The camera turns with the army: screen-up is the army's facing** (designer: "we'll want the camera to rotate with the army" — nobody plays sideways or upside-down chess), so the d-pad is body-relative and the rotate buttons are turn-left / turn-right; the facing is recorded state, so §11's flanking can use it later; v1 still auto-faces at the barrier drop `[2026-09-08, milestone 4c: nothing auto-faces — the barrier drops along the army's facing AS IT STANDS (the king's rank is row 0, his facing arena-north) and the enemy is dealt in front; when enemies walk the map, the trigger conversation decides how a rear or side approach reads]`.
  **THE CONTROLS AND CAMERA SESSION `[2026-09-09, designer-led — REVISES the one rule above and the camera line; PROVISIONAL in numbers, the phone is the judge]`.** Eighteen rulings, in the designer's order, with what each replaces:
  1. **The camera is NORTH-UP on the walk, always.** It turns for a duel (the army's facing up, as milestone 3 built) and turns back after; each cut is wrapped in a blackout wipe, the battle transition. The turn buttons are gone. "Screen-up is the army's facing" now describes the DUEL only.
  2. **The d-pad turns the army, "just like a typical third person video game."** Inputs are WORLD-relative (up is north). The FACING follows the step: a cardinal step faces that way; a diagonal keeps the facing when the facing is one of its two components and otherwise turns to the perpendicular component, never about-face (no ties). A TAP in a new direction turns in place (a move, as a rotation always was); a HOLD walks. Rotation is no longer an input of its own.
  3. **A duel starts with the pieces WHERE THEY STAND** — nothing is summoned into formation (4c's "the stamp is the pattern" is retired; the second PR of the session).
  4. **Nobody is ever behind the king** — held by the pivot (14) and the box invariant (15), not by refusing duels ("a bullshit way to keep a duel from starting"). `[The fifth walk's verdict, 2026-09-10 ("Alright we're almost there. I'm now getting situations where pieces end up behind the king"): STRICT after any d-pad input — the third and fourth walks' one- then two-rank tolerances are retired and the box is whole after every turn. The king WAITS for a comrade that would end behind him; where the comrades can move only through his cell he STEPS ASIDE (one king step level with him or a rank back, never forward) and goes last; a comrade he would wait for in vain is stuck and teleports beside the body. The one residue: a pivot in a nook narrower than the army, where no cell ahead exists.]`
  5. **The camera never jerks to a tapped piece**: a tap marks its moves at the current zoom and the camera stays. The snap-zoom is gone.
  6. **Pinch to zoom, even under integer scaling**: the pinch is quantized to whole k, a cut per step; the mouse wheel on a desktop; the on-screen zoom buttons are gone ("dumb").
  7. **WASD and the arrows walk diagonals** — held-key chords, the direction the sum of what is down.
  8. **Walking feels like Pokémon**: tap turns and hold walks, steps chain with no seam and one input is buffered, a refused step is a wall bump, the battle wipe in and out; pace is tuning on the phone.
  9. **The minimum gap of 2 is between the PAWN LINES** (each side's camp line, the double-step line), never the front-most pieces: pieces pre-moved ahead of the pawns "via exploration pre-moving or pure circumstance" sit inside the gap (the second PR).
  10. **The king makes his own chess move on the walk** — a king step, captures included — "and the rest of the army still gets the auto formation move as if I'd tapped the d-pad."
  11. **Every interaction with the environment is an ACTUAL CHESS MOVE.** A manual move offers a piece's chess moves and nothing else — the king-step option exists only inside the auto-formation move; `^` (crates, doors, everything) is taken only by a chess move, never by a d-pad step (a bump is a bump); a front pawn cannot open the door dead ahead of it — that is the puzzle. Enemy pieces are never captured on the map (§5.2's note: a piece dies only in a duel).
  12. **THE ANCHOR IS THE FORMATION'S FRONT-CENTRE, not the king** (designer: "using the king as an anchor isn't the way… I'd want the pawns to file in first and the king to go last"; the back line will be editable, and nothing in the movement is special about the king). The d-pad moves that cell — it must be floor, so "blocked" means the front met the wall `[AMENDED the same day on the first walk: the anchor may stand ONE cell into stone beside reachable floor, so a single crate or pillar ahead is flowed around, and "blocked" is a step that moves nobody]`; every piece, the king included, walks to its slot `[the same day: the king hurries three when his path is longer than one, and a step that would leave him more than three cells from his slot becomes a REGROUP — the anchor holds, the walk runs, nothing is refused]` `[AMENDED on the second walk's verdict, the same day ("the king in weird spots… multiple spaces behind the rest of the army"; "the pieces should at least be able to stay adjacent if I'm only using d-pad movement"): THE KING NEVER OVERTAKES a comrade and nobody ends behind him — the walk keeps 15's row, not a teleport after the fact; the targets of a walk are ONE CONNECTED SET, unique, molded beside one another and never behind the king's cell, so the formation flows round stone as one body; the leash is TWO cells]` `[AMENDED AGAIN on the third walk's verdict, the same day ("the whole goal of this is to make it so when moving with just a d-pad the army should never be split. They should stay in a battle ready cluster as much as possible"): THE CLUSTER IS THE INVARIANT — after every d-pad turn the army is one body, every piece touching another (a diagonal across a wall's corner does not count); a walk that would split it gives back last steps, the king's first, until it is whole (the army waits for a straggler on its way); a laggard that could not move at all is stuck and teleports beside the body; the king's never-overtake planning rule is retired (it froze the army with two pieces level with him in a nook) — the king plans freely, last in the order, and gives his step back only when a comrade would end behind him and not touching him]` `[The fourth walk's verdict, 2026-09-10 ("they're much better at staying together. but now I'm getting blocked in a lot of places I feel shouldn't be a problem"): the retraction became THE ROPE — the army settled from the king outward, each piece taking the farthest cell on its way that keeps it beside the body, a follow step for one with nothing to touch, the chain giving back a step only then; the body is eight-adjacent, corners included (a target is still never molded across one), so an army passes a crate-and-wall gap one piece at a time; a regroup that moves nobody yields the step's own walk with the anchor held, so the front closes up and the anchor never runs ahead of the army]` `[The fifth walk's verdict, 2026-09-10: the king's plan never holds across the walk's passes (released as "he stays", the comrades take the cells they need and he plans round them, last); a comrade standing on its own target in a piece's way trades targets with it; the anchor enters stone only beside or ahead of floor and snaps to the floor beside a wall it would walk along]`; through a one-wide gap the pawns file in first and the king last by construction; the camera follows the formation's centre.
  13. **CATCH-UP**: a piece whose path to its slot is longer than one step takes two that turn, three while it is behind the king — a pawn's only walking move is one step, the king's own speed, so without this a pawn that lost a step to a pillar could never make it up (the designer's "half the army, usually pawns, trailing way behind"; "imagine an enemy AI having to wait patiently for all the pawns to catch up").
  14. **TURNS PIVOT, walks walk.** A facing change wheels the formation in place before the step: every piece slides straight to the cell it holds in the turned formation, about the king, molded to the ground (the nearest floor at or ahead of the king when its cell is stone), one beat, no pathfinding — a quarter turn moves a piece a cell or two, an about-face swaps the front and back rows. A teleport dressed as a slide, chosen because an about-face in a three-wide corridor is a sliding puzzle that walking cannot solve and turning to face what is behind you will be the commonest turn once enemies hunt.
  15. **THE ARMY ALWAYS FITS THE BOX** — a 10×10 with the king on its first row, "not necessarily centered", slid along his rank wherever it needs to be — after every input. A manual move that would break it is not offered (the box was outlined on the map while a piece was selected until 2026-09-10 — designer: "get rid of the big blue square when I make chess moves during exploration" — the filter stays, the outline is gone). A STUCK piece — no sequence of its OWN moves reaches its slot (a knight beyond a thin wall has its hop back and is not stuck; a pawn waiting in a doorway queue is not stuck) — is teleported to its slot: "just teleport pieces that get stuck somewhere, who gives a fuck." `[On the second walk's verdict, the same day: the row is kept BY THE WALK — the king never steps ahead of a comrade and no comrade ends behind him (a way round a crate pair may dip two cells behind him and come back within the turn) — so the teleport is only for a piece with no way to its target that does not end behind the king, and for what the ten-by-ten cannot hold.]` `[The third walk's verdict, the same day: the walk's row is "nobody behind the king unless one rank behind and touching him" — the queued rook a step behind him at a door — and THE DROP molds such a piece into the box (ruling 4's teleport); the one-body cluster (ruling 12's amendment) is the walk's invariant, and a laggard that cannot move at all is the stuck piece this ruling teleports, beside the body.]` `[The fourth walk's verdict, 2026-09-10: the walk's tolerance is TWO ranks behind the king — a file of three through a gap spans two by geometry — read alike by the box loop, the drop's molding and the manual moves, which are offered while a comrade stands there when they leave the box no worse.]` `[The fifth walk's verdict, 2026-09-10: the tolerance is gone — the box is whole after every input and the manual moves filter on it; a stuck piece is one with no way to its target at all, or the comrade the king waited for in vain at a pocket's mouth once even his sidestep moves nobody, and it teleports beside the body, never onto its own cell.]` Nothing is ever sealed off ("you really shouldn't be sealing pieces off completely anyway"): after a duel, if the king's pocket cannot hold the army, the whole army moves to the nearest floor that can.
  16. **THE DROP**: the box slides to hold every piece and takes the offset that centres the formation when there is slack; the pieces stand where they stand; the enemy molds across the pawn-line gap and around the player's pieces; the one refusal left is a position decided at load. §5.3's trigger reads the same way: four AXES on the king, the box slid to hold the army along each, the enemy king on that box's far row.
  17. **Drag to look around** — "I know pokemon doesn't have that, but I want to be able to look around": a drag pans the camera; it glides back to the army on the next input that moves it. The swipe-to-step is gone ("I do not give a fuck about swipe to move").
  18. **The knight keeps its hop over walls** on the walk — "they still have a legal move to get back, why would we limit that?"
  Build order: the CONTROLS AND CAMERA PR first (1, 2, 5–8, 10–15, 17, 18 — the movement model is inseparable from the controls) `[BUILT 2026-09-09, the same session — play/README.md § "The canvas board", milestone 4d]`; the DUEL START PR second (3, 9, 16) `[next]`.

- **Enemy summoners' armies are likewise visible while roaming** — what they have is what you see (scouting = shopping, §8; level telegraphing is literal). The old "roamers are kings only" rule is repealed; armies still materialize into the §4.2 patch when the barrier drops (v1: both formations stamped auto-facing each other; facing/flanking consequences deferred — §11). `[2026-09-10, THE ENEMIES SESSION (designer: "sounds good, go ahead" on the six picks): AN ENEMY IS THE SAME ARMY — an army.mjs Army on the black side with a pattern from its spawn's digit (the width) and a seeded composition, driven through the walk's own turn planner with step inputs, so the cluster, the box and nobody-behind-the-king hold for it as for the player; its pieces trail its king in formation on the map and are re-molded at the drop (§5.3, ruling 16), so only its king's cell decides a duel. THE PLAYER'S pieces are literal at the drop (ruling 3). Nothing stamps forward any more.]`
- Roamers are **finite per floor, no respawns** — kills the farming incentive, makes clearing a floor mean something.
- **Two terrain classes:** *walls* block movement and LOS; *holes* (Earthquake scars, §4.5) block movement but not LOS — you can see across a pit. Both project into duels identically as `*` wall squares, and the linter (§6) counts both when clipping patches. `[2026-09-08 proposal, pending the designer: FURNITURE blocks sight too — a crate is terrain everywhere but the capture and a door is a door — a one-line switch either way; no range cap and no fog for the first build.]` `[2026-09-10 RULED: yes — walls, doors and crates block sight, holes do not; no range cap, no fog. Sight is KING TO KING on the cell grid, checked after EVERY move.]`
- Patrol behavior while roaming: `[OPEN — routes vs. random walk]` `[2026-09-08 proposal: SENTRIES for the first build — an enemy stands at its spawn until it sees you (a predictable encounter for the phone verdict); routes later as a list of cells in the world file.]` `[2026-09-10 RULED: sentries first. A sentry stands until its king sees yours; sight lost sends it to the last-seen cell, where it stands again. A closed door is a wall to it — automatic moves never take furniture — so the cell block's sentries stay kennelled until the door is opened; crate-smashing enemies stay held over.]` `[2026-09-11 RULED, on the first phone logs: ANY TWO PIECES — a sentry stands until ANY piece of its army sees ANY piece of yours (designer: "maybe the line of sight is too strict. Maybe we should count it as any two pieces seeing eachother, not just the kings" — "Do both, go ahead"); the last-seen cell is still your king's, since the hunt's goals are his boxes.]` `[2026-09-11 RULED, the same day, on the far-half build ("Alright seems to work a lot better. Can we get some wandering enemies?"): WANDERERS. Every spawn ROAMS by default — a waypoint walk on its BEAT: a random floor cell its king can reach within a LEASH of 12 cells of its spawn (the level telegraph stays where the generator put it — a far spawn's army never drifts to the start) and at least 4 off, a pause of 2–6 turns at each, every draw from the enemy's own seed by its count, so a run replays from its inputs; speed parity, one step per input; sight after every move, so a wanderer that walks into view of your army hunts at once, and one whose search finds nobody goes back to its beat. Two wandering armies are STRANGERS to each other — they share the lowercase letters, and the walk now blocks on any piece that is not the army's own. `?enemies=sentry` keeps the old rule (the labs, the smokes); patrol ROUTES as a list of cells stay open.]`

### 5.2 Detection & pursuit state machine `[LOCKED]`

```
ROAM ──(line of sight to player)──► HUNT      [ROAM = the WANDER, 2026-09-11: a waypoint walk on the spawn's beat]
...
- Detection is **simple line of sight**, decoupled entirely from duel geometry. Walls and corners are the stealth system. `[OPEN: LOS range cap, what blocks LOS beyond walls]` `[2026-09-11: sight is between ARMIES — any piece of one seeing any piece of the other (at most 64 rays, checked after every move). Measured on the generated floors before the change: the kings saw each other from 2–3% of enemy-and-position pairs, any two pieces from 7–12%, and from farther away (median ten cells against seven); around the two spawns of the first phone logs the enemy king saw the player's from 128 and 66 of about 540 nearby floor cells, any piece any piece from 344 and 220.]`
- The pursuer chases a *relationship* (a duel-legal square band), not the player's tile — so persistent pursuit converges even though equal-speed tag never catches anyone. Evasion buys time and choice of ground, not immunity. `[2026-09-08 proposals (CLAUDE.md § Phase 2, milestone 6): the band is the FAR ROWS of the player's four boxes (§5.3); the hunter BFSes its king to the nearest one and takes the first step as its king step into the army rule, never turns, and walks toward the player's king when no legal cell is reachable; sight is checked after EVERY move, so a step into a clear aligned line at nine is the player's ambush; speed parity is one enemy turn per player input, waits, turns and individual moves included.]`

### 5.3 Duel trigger condition `[LOCKED]`

`[REWRITTEN 2026-09-08, THE TRIGGER CONVERSATION — designer: "just make it always a 10x10 arena. As soon as the two armies fit in a 10x10 box and it's a legal board state with both kings in the right rows, then the duel can start." YOUR BOX: four 10×10 boxes sit on the player's king, one per world direction, his rank the box's row 0, centred on his file (four to the left, five to the right; nothing slides — 2026-09-09, on the first log). A duel starts the moment a hunting enemy's king stands on the FAR ROW of one of them and the deal is legal: both armies mold into the box (fit means the molding fits, not where the walking pieces stand), gap ≥ 2, the armies connect, the lint passes. "Anywhere on the far row" is read from "both kings in the right rows" — item 1's band alignment is back and 4c's strict colinearity retired — and is NOT YET CONFIRMED by the designer `[CONFIRMED 2026-09-10, the enemies session: THE FAR ROW IS A BAND — any file of it — because ten target cells per axis are what make the chase converge]`. Items 2–4 below are history: the distance is always 9 and the arena always 10×10. The same function feeds the hunter's targets, the live check, the threat display and the generator's lint (§6).]`

`[AMENDED 2026-09-09, THE CONTROLS AND CAMERA SESSION (§5.1 rulings 3, 9, 15, 16): the box is not centred on the king's file — it SLIDES along his rank to hold every piece of his army (the army always fits one, §5.1 ruling 15) and takes the offset that centres the formation when there is slack; the pieces start WHERE THEY STAND, nothing is summoned; "fit" means the enemy's molding fits across a gap of at least 2 measured BETWEEN THE PAWN LINES (each side's camp line), with pre-moved pieces inside the gap and the enemy molding around them; the one refusal is a position decided at load. Four AXES on the king, one per world direction, replace "four boxes centred on his file". Not yet built — the session's second PR.]`

`[2026-09-10, THE ENEMIES SESSION — three more rulings on the trigger, built to: (a) A REAR OR SIDE CATCH PIVOTS YOU — your pieces are literal at the drop, so an enemy on an axis other than your facing would find your army behind your king along it; the drop wheels the army to face the axis first (ruling 14's pivot, the same motion a tap-to-turn makes, molded to the ground) and then reads the pieces where they now stand; flanking consequences stay §11. (b) SPEED PARITY: one enemy turn per input that costs a turn — steps, waits, turns and manual moves, never a refused step — your turn first, then each enemy in spawn order, the trigger checked after each army's turn, every arrival in ONE slide so the held pad keeps its cadence. (c) INITIATIVE per §4.4: the side whose move completes the alignment moves first; several at once opens the chooser. A cell of the far row where the deal fails the lint — your rook covering that file, say — is not a target: the hunter walks round your rook's file and the threat display shows why.]`

`[2026-09-11, THE FAR HALF — designer, on the first phone logs ("First one had a lot of trouble starting the duel"): "Do both, go ahead". The enemy king triggers from ANY ROW OF THE FAR HALF of a box — rows 5 to 9 — on a file whose deal is legal; the deal molds it onto the far row as ever (its walking pieces were never read, ruling 16, so its standing cell only names the axis and the file). WHY: the far row alone made a RETREAT DANCE — a hunter that first saw you inside nine had to back off to nine at speed parity, and a player walking at it kept the distance forever; measured, of six charges that began under king-to-king sight four never started in 250 turns while the enemy retreated 15 to 86 times, and the same six with a wait pressed after first sight started within ten turns. With the band, walking at an enemy starts the duel the moment the kings are nine apart, your initiative; a hunter noticed at seven starts it at once; only a sentry noticed inside five backs off, to five. "Both kings in the right rows" now reads: the player's king on row 0, the enemy's anywhere in the far half, dealt onto row 9. The pivot was measured first and acquitted — a rear or side catch turning the army is ruling (a) working, never refused at 1,200 placements and 2,246 walked turns. After: sixteen charges on the fixtures, nine sighted, nine started (wait after first sight) or eight (walking on), the dance zero. The phone, the same day: "Alright seems to work a lot better" — in.]`

A duel starts when, after any move, all of the following hold between a hunting summoner and the player:

1. Their prospective patches are **aligned along one axis** (band alignment — king anywhere in his back row; no strict colinearity). `[AMENDED 2026-09-08, the barrier by hand (§10 Phase 2 milestone 4c): the designer's rule for the barrier is "gap 4 for now, and the kings have to be aligned" — read as STRICT colinearity, the enemy king on the player's king's file on the arena's far row; the trigger conversation may loosen it back to the band.]`
...
- **Width 1–2 passages are non-duelable crawlspaces** — no room to raise an army. Author them scarce so they read as claustrophobic exceptions, not safe hallways. Width ≥ 3 is duelable with clipping: tight ground doesn't block fights, it **eats your formation**. Terrain-modulated material, one ruleset. `[RETIRED 2026-09-08 — designer: "WHY NOT JUST NOT USE 2 WIDE HALLWAYS": the generator (§6) authors no passage under 3 wide, so the clause has nothing to apply to; under the fixed box a 2-wide passage would otherwise have been duelable, the kit molding three deep in it.]`

### 5.4 Threat display `[LOCKED in concept]`

While hunted, highlight the tiles where a duel could legally complete, shaded by **clip severity** (which formation slots would survive there). Into-the-Breach-style legibility: one glance says "if he catches me in this hall, I fight at 3-wide." `[2026-09-08 proposal: the FAR ROWS of the four boxes (§5.3), lit where the deal would be legal and dim where not — forty checks per move; molding replaced clipping so "clip severity" has no referent, and the box is always 10 wide, so the arena you would fight on is the map around you.]` `[2026-09-11: THE FAR HALF lit — every cell of rows 5 to 9 of a box on a file whose deal is legal, the far row as a frame and the rest of the band as a tint (§5.3, the far half).]`
...
- **Per-tile linter:** stamp-check whether a ≥3-wide patch fits at each tile, per axis (pure local check — no line-run scans needed since colinearity was dropped). `[2026-09-08: colinearity is back for the barrier (§5.3 item 1 as amended) — a line scan along the king's file returns to the linter when the trigger pipeline is built.]`
...
- **Encounter enumerability:** because the trigger is deterministic, every (roamer, spotting geometry) → arena instance is knowable at map-gen time. Generated maps can have sampled arenas validated (winnability + pacing) before acceptance. **v1 sidesteps this with hand-built maps.** `[OVERRULED 2026-09-08 — designer: "randomized dungeons are a requirement. Not just one alg either, I need different floors to have unique styles and features and themes"; both hand-built maps retired on measurement (no 10×10 crop of either comes near the wave-6 bed — CLAUDE.md § Phase 2, "The trigger conversation"). THE GENERATOR is Phase 2 milestone 5 (§10); its lints are measured off the bed rather than guessed: NO BOX IS BORING (every 10×10 box the trigger could drop has a largest empty block under 30 cells, at least three separate terrain features and about 40% of its floor touching something), THREE WIDE EVERYWHERE (the floor eroded by one is one connected body and every floor cell is within a step of it), reachable, NOTHING SYMMETRIC, and DUELABLE GROUND checked by the trigger function itself. Skeletons × room recipes × passage recipes × a wear pass × one signature piece, five styles, themes riding the style, seeded from the run, the same module in the harness with a gallery for the eye.]`
...
- **Pawn promotion reachability `[DEFERRED 2026-09-01 — measure first; was NEW/unbuilt]`.** The linter checks that walls don't sever the two formations; it does **not** check that pawns can reach a promotion rank. They frequently can't: 5 of 18 pawns across the shipped Phase 1 arenas are push-blocked from move one, and at generated wall densities 0.15–0.3, **95.6% of positions carry at least one terrain-locked pawn** (mean 4.18). This is the single largest source of inert boards, and the Director can only paper over it — displacement frees such a pawn 12.7% of the time per attempt, so clearing a board's worth costs more Earthquakes than the design can afford (§7).
  - **Why deferred:** every number above is v2-era, and v3 was built partly on that data — the ladder attacks the lock DIRECTLY (weaken gives +3 impact to walls on a locked pawn's file, breach scores by locked pawns actually freed, and staleness weights pawn-lock into the fill rate, §4.5), while the bed moved from density-rolled generation to designer-locked stages. The god lab records the locked-pawn trajectory per game (start → end — "the Director's actual job, measured directly", §7); if v3 clears locks in the corpus, the lint stays dead, and if boards still go inert it returns as a GENERATOR lint in Phase 2, when a generator exists to serve. Same move as the deferred "no new winning capture" rule: see whether the problem still exists before writing law against it.
...
- Sweeps must simulate the Board State Director (§4.5) — which requires the same mid-game position-surgery mechanism as live play (spike 11). Director RNG seeded per game for exact replays. **✅ Solved structurally 2026-09-01: the god lab (`phase0/harness/godlab/`) drives the CANON `play/js` DuelController + Director on the locked stage bed rather than simulating them** — the planned `game.mjs` port was cancelled as drift-prone (two implementations of the shipped loop is this bullet's own validity rule waiting to fail; `play/js/duel.mjs` already IS the ported loop) and the crumble-era harness retired to `harness/legacy/`.
...
- **No attrition `[LOCKED]`:** pieces captured during a duel by the enemy are restored after a win `[2026-09-01: "or by a collapsing floor" is gone — quakes cannot swallow pieces, §4.5]`. Duels are all-or-nothing: win and the army walks out whole; lose and the run is over. The only stake is everything.
...
- **Phase 1.3 — Redefine "symmetric" `[NEW, blocked on 1.2.5]`.** **Scope note (meter-lab first pass, `phase0/results/meterlab-findings.md`):** the completed evidence pass found the *trigger* was the wrong half — `pQuake` is a function of ply alone, and a state-aware "restlessness meter" cut per-game god-inflicted harm ~55% with the filter stack untouched. So this phase may widen from "redefine symmetric" to **replace the decision layer** (meter trigger + effect rules + possibly dropping the pairing rule). §4.5 is `[LOCKED in shape]`, so that widening needs a design conversation before any commit rewrites it. The original scope follows `[✅ PROMOTED 2026-09-01 — the deferral's see-it-in-play condition fired (a breach exposed a queen); shipped as threat.mjs `editExposes` on breach/displacement/crumble, ray-scoped SEE per edit; measured in `results/godlab/tuned-ab-findings.md`, including the answer to the SEE-sufficiency question below: SEE-per-edit kills the calm/restless flip classes and cannot see wrathful's composite/race flips]`. Promote the Phase 1.1 stopgap to the full rule: a quake may create no new winning capture for **either** side, judged on the composite post-quake position. Closes what landing safety alone still misses — discovered attacks from the vacated square, rescues of already-hanging pieces, pinned "defenders" — and settles whether SEE alone suffices or the engine eval delta has to become a gate (SEE is blind to mate-net changes, which the prototype sweeps measured as the dominant flip mode). Expect the crumble rate to move: a stricter displacement filter pushes more quakes down the fall-through path. Retune the presets after. The 1.2.5 evidence now includes the furniture arm (§4.6), so the rule is decided once, on the full board vocabulary — "no new winning capture" and the SEE-sufficiency question must price `^`-captures too.
- **Phase 1.5 — Director calibration `[ACTIVE — rig ✅ built 2026-09-01; runs follow the ladder settling]`.** The rig half is done and it REPLACED the original "port `harness/game.mjs` to `director.mjs`" plan rather than executing it `[designer 2026-09-01 — "fix the doc while you're at it"]`: a port would leave TWO implementations of the shipped duel loop to drift apart, which is the §7 sweep-validity law's own failure mode, and `play/js/duel.mjs` already IS the ported loop. Instead the **god lab** (`phase0/harness/godlab/` — run.mjs, analyze.mjs, sweep configs) drives the canon DuelController + v3 Director on the designer-locked stage bed via `dealMatchup`, both orientations, stratified spreads; the crumble-era harness is retired (`harness/legacy/` + the results retirement note). Corpus lines record `variantName`/`variantIni` (fixing the meter-lab rig's replay defect by design) and the multipv favored seat carries the fresh-engine recovery the meter lab lacked. Its metric list is the first v3 phone report turned into numbers (designer 2026-09-01, a few games: **even Calm + intensity 1.0 reshapes whole arenas — breach-heavy, rooms stripped of furniture fast; "the chaos is pretty fun, but I wouldn't call this calm"**): terrain-attrition curves vs authored counts, time-to-first-quake and actions/100 plies per preset × stage class (core/rooms/floorplan), per-ply staleness + pressure + locked-pawn trails, and the §7 alarm referee. Remaining, in order: (a) the **baseline corpus** — ✅ a preliminary 192-game pass ran 2026-09-01 (`results/godlab/prelim-findings.md`: gods-off never terminates on 42% of the bed; Calm fires 3× harder on floorplans than core, so stage class outweighs the preset dial; terrain strips to 8–24% remaining; flip quakes characterized in its §7 — strip races re-timed, lock-break conversions, early remodels, none of them SEE-visible); the full `presets.json` grid stays available for tighter error bars; (b) the **terrain-context change** — ✅ BUILT and A/B-measured 2026-09-01 (§4.5's new conservation brake, `results/godlab/brake-ab-findings.md`: calm floorplan terrain remaining 18%→32%, pacing and alarm flat, displacement share up as the fall-through; `conserveAt`/`conserveFloor` are live dials for feel-tuning); (c) **preset separation — ✅ first pass 2026-09-01**: presets now carry per-preset staleness knobs (`GOD_PRESETS` moved to `director.mjs`, one copy for game + labs); measured separation 5.9 / 11.7 / 26.1 quakes/100p (was 12.3 / 14.3 / 20.8) with calm keeping 43% of authored terrain and a 0.3%/quake flip rate (`results/godlab/tuned-ab-findings.md`); the contact-term rework ("staleness waits for first contact") stays open if calm still misbehaves on floorplans; (d) settle the ramp numbers from rig + phone feel together — **phone verdict 2026-09-01: calm is "finally suitably chill", settled**; restless untested on-device; wrathful reads as crazy (its brief), with its extra heat under the exposure guard the one number still open. Also shipped in this pass: **quakes cannot swallow pieces** (§4.5 crumble rung, `results/godlab/no-swallow-ab-findings.md`). **Phase 1.5 wrapped for merge 2026-09-02**; what remains is listed under Phase 2's gate below. The favored-seat model and edge size (carried over from the shelved 1.2.5 rig) still need the designer before the live-regime arm burns compute. The promotion-reachability lint moved out of this phase — deferred pending the locked-pawn trajectory data (§6).
- **Phase 2 — Exploration slice** `[NEXT — designer 2026-09-07: "I've been putting off Phase 2 long enough"]`. **It opens with the rendering pipeline (§2 item 5):** one full-board buffer at native 16-px-per-tile resolution, every layer written into it by the existing pure painters, scaled to the screen once at an integer device-pixel factor (the board no longer fills the viewport exactly and steps in size — the pixel-art norm), with a camera for the 100×100 floor. Decided after the tile-grid piece work of 2026-09-07 showed the DOM board's cost: every layer is a separate image the browser resamples at a fractional scale, so alignment is a property each layer must re-earn by construction (cell-sized boxes, measured row rectangles, position baked into tiles, a two-browser gate), the debris image sits a sub-device-pixel off the floor, slides shimmer off-grid, and a DOM grid of ten thousand cells cannot carry the overworld. The spike comes first: a full-board canvas at integer scale running the existing painters, judged for flicker on the designer's Firefox/Windows and Android Firefox before anything is built on it (the "no canvas on the board" rule was written against PER-CELL canvases mixed into DOM compositing — one board-sized surface is a different animal, but the phone decides). What carries over: the pure modules (`piecetiers`, the debris ledger + painter, `classifyTerrain`, the autotile masks, the atlas and the repack tool — one atlas PNG instead of data URIs), the rules and the Director. What is rewritten: board-ui's DOM painting, style.css's tile rules, the arrow/mark overlays, the FLIP slides and quake fx, the smoke suites' renderer checks; the replay analyzer shares the new renderer. Then the slice itself: one hand-built map, two summoners with different levels. Army-as-avatar movement (blob + marching pattern, visible enemy armies), LOS/hunt/pursuit state machine, threat display, trigger → barrier → FEN pipeline proven end to end.
  - **Milestone 1 ✅ built 2026-09-07 — the canvas board, behind the Renderer option** (`play/js/canvas-board.mjs`, `play/README.md` § "The canvas board"): not a throwaway spike but the real renderer's core — the atlas loader, the native buffer, the one integer-scale blit sized from the device-pixel content box, on-grid slides, the terrain fx with held end frames, the rumble, the flight as buffer pixels — running the existing pure painters on the Phase 1 duel page with the DOM board's method surface (main.mjs unchanged beyond a factory and the option). Gated tile-for-tile against the DOM board (`canvas-parity.mjs`: exact), on the device-pixel grid in Chromium and Firefox (`canvas-grid.mjs`), by the live smoke on both renderers and the selftest. **The phone decided (same day): "works fine on both desktop and mobile" — k 3 on the desktop, k 6 on the phone — and the fill scaling "doesn't look bad either"; the one report, "a big white rectangle flash", was laid at the SVG arrow overlay, so the arrows are pixel art in the buffer now (`play/js/pixelarrow.mjs`) and nothing overlays the canvas; whether the DOM board is retired is the next verdict — ANSWERED the next session, same date: "looks fine with canvas rendering on both desktop and mobile, including integer scaling". The DOM board is RETIRED — **Milestone 2 ✅ 2026-09-07, the same session**: one PR of deletion plus one move (the in-house drawings into the atlas's `classic` row, the debris sampler and the options legend reading the atlas, the residue rule on data, the replay page on the canvas board; the DOM class, `tiles.css`, the piece tiers, the classic glyph piece set, the % dials, the parity and piece-grid gates gone), gated pixel for pixel on six cases before and after — every pack theme identical, and the classic theme FIXED (the canvas board had drawn its SVGs at their 150-px decode size). The camera is the PR after it (the buffer becomes the viewport, k on both axes, the camera owns the desktop screen, fill duel-only, facing in the camera model with the turn buttons waiting for the army, a generated edge-on door placeholder); CLAUDE.md § Phase 2 carries the full handoff.** Three decisions from the same session, recorded here: **the duel is a camera view of the same world, zoomed** — the barrier drops on the map you were walking, the camera zooms to the largest integer step that fits the arena with a margin, the dungeon outside dims (§1 read literally; it deletes the letterbox problem, makes holes and debris persist with no transform, and means one renderer with no board mode; the stage bed survives as fixtures that load as small worlds); **the camera turns with the army** (§5.1), which costs the tiles nothing because every direction-bearing tile is generated from a mask computed in SCREEN space (wall faces, ruin stubs, pit rims, doorway posts follow the turn; debris buffers rotate by index permutation; pieces and props are never rotated; variant hashes key on world coordinates; the turn is a cut, not a smooth rotation) except the DOOR, which needs edge-on art per theme (the pack survey found none — a gap in the wall's top band with the leaf as a thin slab and a post above and below, the double as a two-tall pair, the doorway's north–south post variant generated); and the next milestones in order — the camera and rotation with the edge-on doors, then the world and the army under §5.1's one rule with one hand-built map (a walk-around build on the phone before any enemy exists), then enemies, LOS, hunt and the trigger pipeline.
  - **Milestones 4a + 4b + 4c ✅ built 2026-09-08 — THE WORLD, THE WINDOW, THE ARMY, THE WALK, THE BARRIER BY HAND** (CLAUDE.md § Phase 2 carries the record; `play/README.md` § "The canvas board", milestones 4a–4c). 4a: the world's data and the crop transform, the board a window over a world grid. 4b: §5.1's one movement rule on a hand-built floor, the run save (one object per run, no meta progression, no backward compatibility), the phone verdict "seems to work fine on mobile and desktop". 4c: a debug button drops the barrier on the army as it stands — the crop of the floor under the army's facing, the room's width across the king, GAP 4 with THE KINGS ALIGNED on one file (designer 2026-09-08), the depth computed from the molding, the carried pattern stamped whole with the royal pinned, the enemy dealt across the gap; the duel runs on the world (every ply writes the crop; the world's pits and cracks are the gods' ledgers from ply 0), the run records the duel as its result and re-drops a duel it was reloaded under, a win walks the army out whole around the king's final cell, a loss ends the run, the debris ledger lives in the run and the walk's smashes and steps scar the floor, the replay log carries the world. No trigger, no enemies on the map, no line of sight — those open with the trigger conversation.
  - **Milestone 5 — THE GENERATOR `[decided 2026-09-08, the trigger conversation; ✅ FIRST PR BUILT the same day — the box, the lints measured off the bed, the prefab-grid skeleton laying the 36 arenas as pieces, the fixtures vaults-1…4, the setup screen's New floor; `play/README.md` § "The dungeon generator"; the recipe skeletons are the next PR; 2026-09-09, the designer's direction after the first floor: categorized pieces used intelligently — a 5-cell lattice of tagged pieces in several sizes, a floor PLAN drawn first, seams exact where the plan connects, the recipe generators as piece factories; a controls-and-camera session comes before it; CLAUDE.md § Phase 2 carries the answer on record]`.** The conversation opened as planned and turned on the ground: the designer ruled the arena ALWAYS 10×10 with a minimum gap of 2, no hallways under 3 wide, and — on the measurement that no 10×10 crop of either hand-built map comes near the wave-6 bed — both maps retired and a generator with several algorithms and per-floor styles made the milestone ("randomized dungeons are a requirement"). The plan, its lints and its five starting styles are in CLAUDE.md § Phase 2, "The trigger conversation"; the first PR is the lints, the prefab-grid skeleton that lays the 36 arenas as pieces, and the gallery.
  - **Milestone 6 — enemies + LOS + the trigger, on generated floors.** The trigger is the fixed box (§5.3 as rewritten); the hunt, sight, sentries, speed parity, initiative, no captures on the map, the engine booting with the run and the save's shape are the conversation's proposals, recorded in CLAUDE.md and built to unless the designer says otherwise. `[✅ FIRST BUILD 2026-09-10/11, THE ENEMIES SESSION — the six rulings in §4.2 / §5.1–§5.4 and CLAUDE.md; `play/README.md` § "The canvas board", milestone 6, is the record: the 4×2 kit, the duel start (rulings 3, 9, 16), `play/js/enemy.mjs` — sentries, king-to-king sight, the hunt to the far rows of the four boxes through the trigger function itself, the trigger with its initiative, the chooser, bystanders, the save. The phone verdict is the gate.]` `[2026-09-11, THE FIRST PHONE LOGS: sight is between ARMIES (any piece seeing any piece) and the trigger is THE FAR HALF of the box (§5.1, §5.2, §5.3, §5.4) — the retreat dance the logs showed is gone.]`
  - **Milestone 3 ✅ built 2026-09-08 — THE CAMERA**
 (`play/js/camera.mjs` the pure geometry, the canvas board through it; `play/README.md` § "The canvas board", milestone 3; CLAUDE.md § Phase 2). FACING is in the camera model — which world direction points up the screen, 0 north … 3 west, `flipped` the old spelling of 2 — and a quarter turn is a CUT: the buffer swaps its axes, every square, pixel, mark and arrow goes through the one origin function, the world-space autotile masks (still the shared test surface) are PERMUTED to the screen before the tile lookup so wall faces, ruin stubs, pit rims and doorway posts follow the turn, a square's debris turns by index permutation, pieces and props never turn, and the cosmetic hashes and the checker key on WORLD coordinates (the environment's cell, the debris ledger's space) so a crop, a flip or a turn never reshuffles the floor. THE CAMERA OWNS THE SCREEN on a wide screen (main.mjs stamps `body.layout-wide` at one breakpoint, 900 px): the board is an explicit box the canvas fills, k the largest integer step that fits the board AND its headroom row on both axes — a 1080p desktop goes from k 3 to k 5, height-bound — the board centred in whole device pixels, the panels in a column beside it; a phone keeps the stacked layout the verdicts were given on. Fill is the exact quotient of the tighter axis. EVERY DOOR IS A DOOR (§11): the leaf across the screen, the generated edge-on placeholder up it; doubles pair along both axes and their halves are dealt on the screen. The turn buttons wait for the army: the debug pair in Options → Look, `?facing=`, `__DCK.renderer.facing()`. Gated pixel for pixel: `camera-guard.mjs` (six cases × fifteen plies of a hot duel recorded on the build before, replayed from the recorded INPUTS on the build after — north-up byte-identical except the door squares, south-up identical except the art that legitimately turns), `facing-walk.mjs` (all 36 arenas × three facings: the turned camera equals the world itself rotated, 108/108), `test-camera.mjs` (the geometry against brute force, 80 checks), selftest 44/44, ui-smoke 213 (the turn through the real buttons, every square's hit-test round-tripping, the wide layout at 1280×720), replay-smoke 63, canvas-grid 4/4 Chromium + 18/18 Firefox. The world beyond one arena — the buffer as a viewport over a world grid — is the FIRST step of the next milestone, since there is no world to paint yet. The desktop look is the designer's to judge on this build.
- **Phase 3 — The loop `[OPENED 2026-09-25 with THE DECK — §4.10; numbered the same day, designer: "give these future steps phase numbers that fit in with the current overarching plan"]`.** Rewards, collection, level scaling, floor transitions, the first full runs — and the collection IS the deck, so Phase 3 opens with spells as cards. The three spells that preceded it (portals, the sledgehammer, the ice — 2026-09-17…24, §4.7–§4.9) are its prologue. Phase 2's open items (the generator's plan layer, the walk's revisit list) stay Phase 2 and interleave as the designer sees fit.
  - **Phase 3.0 ✅ 2026-09-25 — the record.** §4.10: the rulings, the measurements, the staging.
  - **Phase 3.1 ✅ 2026-09-25 — the deck.** `play/js/deck.mjs`, the refill in duel.mjs, the run's deck, the log's decks, the first card row, Reveal and Undo, the redraw; no engine change. `play/README.md` § "The deck".
  - **Phase 3.2 ✅ 2026-09-25 — THE CARD UI `[designer: "These little buttons aren't gonna cut it. I wanna take notes from other card wielding games like mtg arena and hearthstone. Probably cast spells by dragging them from the hand to the field"; the proposal in §4.10 "The card UI", its four calls agreed — "Looks good. Agree with all 4 calls" — and built the same day; `play/README.md` § "The card UI"]`.** The hand as a fan of cards under the board, drag to cast with the targets lit and an area's shape under the finger, tap to cast kept, a long press to read, the enemy's hand face-up in its bar and its played card shown for a beat, the draw from the deck stack, the piles, the redraw from the deck sheet behind a confirm. Game layer only; `play/js/cards.mjs` the pure half. The phone verdict is the gate.
  - **Phase 3.3 — TERRAIN EDITS, the first forge.** One patch: a cast that edits terrain by HITS (intact → cracked → floor, bedrock immune, pieces untouched) in shapes (a square, the 3×3, a line, a cross) — Crack, Smash, Demolish, Blast, Lance — plus Sink, Raise, Harden; the sledgehammer as an ENCHANTMENT (a per-colour flag a cast on the king's square switches on, retiring the always-on setting); a per-type drop region for Reinforcement; Golem by ini. A cracked wall from a card joins the one crate ledger (§4.8).
  - **Phase 3.4 — TIMERS AND PIECE STATES.** The countdown in the state, the FEN and the hash; Stun (cannot move, still attacks and defends, capturable; single and area), Entomb (inert, uncapturable, a wall to lines, freed by any hit, melted by fire), Ward, melting ice, Palisade. Kings never targets.
  - **Phase 3.5 — MATERIALS.** 3.5a without fire: the per-square material and the transition table — Water, Oil, Lightning (a shocked body of water kills, both colours; your own king in it makes the cast illegal, theirs a win). 3.5b FIRE: the tick once per full turn onto orthogonal fuel, lethal at the tick, burn durations, scorched stone, barrels as explosive furniture the generator places.
  - **Phase 3.6 — THE RUN'S ECONOMY.** Gust on the slide trigger; cards from chests opened by a chess move on the walk (§5.1 ruling 11), loot from a beaten enemy's deck, the starter deck chosen at the run's start, deck strength by the enemy's width, dungeon puzzles as a reward source (the designer's list).
  - **Phase 3.7 — BALANCE BY RULES, and the gods to a preset.** Copies, range, area, hand size, enemy deck size, lethal or lesser — set with the library at twenty-plus kinds, read by `deck-stress.mjs` and the god lab with decks; the ladder's stirring moved to a chaos preset once the closer's firing rate is measured (§4.10 "The gods").
  - **Phase 3.8 — FLOORS.** Stairs down, the floor transition, the run across floors, level scaling — the rest of this phase's original list — and the first full runs.
  - **Phase 3.9 — PIECE IDENTITY.** The twin and the family it opens (a state that follows a piece: a bounty, a haunt, a ward by piece); unordered against 3.8.

---

## 11. Open Questions

- Player overworld movement rules; speed parity with hunters; blob deformation/reformation rules (how the marching pattern squeezes through crawlspaces and reforms). `[2026-09-07: the one rule in §5.1 is the proposal on record; open inside it — whether rotation costs a turn, whether the king may make an individual move, how tapping a target works at exploration scale (tap the piece, snap-zoom to the duel scale for the target pick is the proposal), and a rotated army walking a 2-wide passage sideways in formation.]` `[2026-09-08, milestone 4b built the rule: a rotation COSTS A MOVE, the king makes no individual move, a tapped piece snap-zooms to at least k 6 for the target pick; the sideways 2-wide passage is what the fixture's crawlspace is for.]` `[2026-09-09, THE CONTROLS AND CAMERA SESSION — RESOLVED by the eighteen rulings in §5.1: the anchor is the formation's front-centre, the facing follows the d-pad, turns pivot and walks walk with catch-up, the army always fits the box and a stuck piece is teleported, manual moves are chess moves only, the king moves by his own chess move with the army following, no snap-zoom, pinch and drag on the map. Still open inside it: the pace (Pokémon walks at about a quarter second a tile, runs at half that; the phone decides), and whether a tap-to-turn should be free (built as a move).]`

- Edge-on doors (§10 Phase 2): a door in a wall that runs up the screen after the camera turns needs art of its own per theme; the weak-spot rule (a door in a north–south line reads as cracked stone) cannot survive a rotating camera, since the same door would flip between a leaf and a crack. `[2026-09-08, the camera milestone: the weak-spot rule for doors is RETIRED — every door is a door, drawn as the leaf when its wall line runs across the screen and as a GENERATED PLACEHOLDER when it runs up it (the wall's band with the leaf as a thin slab in the leaf's own two tones and a post above and below in the doorway's post tones; a double edge-on is two such tiles, the brief's "two-tall pair"; an opened north–south doorway is the doorway tile turned a quarter, its posts above and below). Still open: per-theme edge-on ART to replace the placeholder — the designer's call on the first build.]` `[RESOLVED 2026-09-11 — THE DESIGNER'S OWN SPRITE ("Use this one"): a 5×16 side-view door in pixel-poem's timber (phase0/lib/inhouse/door-profile.png), recoloured per theme exactly as the face-on leaf is, standing in the gap it is — the walls above and below end with their own autotile end cases, the brick face on a south end, since a door or a doorway whose line runs up the screen is not solid for the wall masks around it (canvas-board #wallMask, in screen space) — and drawn as furniture in the tall pass; opened, the gap between the same two wall ends remains; the door set option swaps the leaf, the posts stay the wall's; the generated slab and the quarter-turned doorway are gone — CLAUDE.md § "The edge-on door", play/README.md § "Art themes", round 20.]` `[2026-09-12, TALL WALLS: every wall is a 16×24 sprite in the tall pass — the Catacombs frame's north band as its roof, eight rows of it standing over the square north, the pack's brick face on the square three pixels off its seam; the hall, the castle and the classic set wear the same drawing as palette swaps; either leaf stands two pixels above the face's foot, at its own square's depth, no roof over a face-on one (the edge-on leaf was stretched to 27 rows on the first build, the far face's top to behind the near roof, and read as a door hung from the roof with no foot to show when the lower half of a stacked double broke — the second round the same day put the designer's tile back as drawn, and the third raised it: its head seven rows up the far wall's face, its foot under the near wall's roof — "that's WAY too low on the wall" at the face-on lift); a thick wall's top is the pack's own frame since that round, a bevelled rim around a void that ramps to black over six pixels from every rim (the third round: "the roof darkness needs to be on a gradient"), where the first build tiled the band over every cell; THE PALETTE ROUND the same day — every roof in its face's own stone, the mortar lifted so a crack reads, floors in browns, tans and dark greys only (the designer: no maroons, no greens, no light greys like the pieces), and Options → Tones, a floor and a wall colour per art set retuned live and saved, so a palette is the designer's pair of hex values; the east–west doorway's generated posts are retired too — every opened doorway is a gap the walls end at — CLAUDE.md § "Tall walls", play/README.md § "Art themes", round 21.]` `[2026-09-15, THE SHORTER FACE AND THE MOSS: the wall's face lost its top brick course (the sprite 16×20, standing seven rows into the square north; the roof and the foot as they were — "just crop the face of the wall, do not fuck up the roof"), the door leaves' lifts became dials in Options → Look (a sixteen-row leaf in a twenty-row wall; defaults 3 face-on, 8 edge-on, off crops), and the tones grew a third slot, THE MOSS — the highlight flecks in the brickwork, which the first tone rule had turned green under any wall colour of another hue; the rule is hue-true now and the flecks take a colour of their own — CLAUDE.md § "The shorter face and the moss", play/README.md § "Art themes".]` `[2026-09-16: the third slot is THE WALL HIGHLIGHT — one colour for the roof's lit line and the brick flecks, "moss" no more ("make it effect the highlights on the roof bricks as well"); the crypt's baked palette is the designer's own slate, floor #5b5b62 · walls #333844 · highlight #3f4555 ("these are the values I like the most right now. Let's make these the default"), the set every generated floor wears; and the edge-on leaf stands from the far wall's face to one row under the near wall's roof, edge lift 6 since 2026-09-17 ("Vertical doors are misaligned again"; on lift 7, "still misaligned. It needs to be moved down one more pixel") — CLAUDE.md § "The wall highlight", play/README.md § "Art themes".]` `[2026-09-17, THE CRYPT EVERYWHERE: Options → Art set defaults to Crypt on every stage and floor ("make crypt the default look everywhere for now. Hall and Castle look like shit but I'm just tired of messing with the aesthetics for a while"); the stage's own theme is an explicit choice, the hall and castle sets stay in the atlas for later — CLAUDE.md § "The crypt everywhere".]`
...
- LOS range cap and occlusion rules. `[2026-09-08 proposal: no cap, no fog, crates block — pending.]`
- The trigger's far row (§5.3 as rewritten 2026-09-08): the enemy king anywhere on it (the band) or on the player's file (4c's strict rule) — read as the band, unconfirmed. `[CONFIRMED 2026-09-10 (any file of the far row) and WIDENED 2026-09-11 to THE FAR HALF (any row 5 to 9, §5.3) — closed.]`
- The generator's designer inputs (2026-09-08, defaults in CLAUDE.md if unanswered): floor size, the arenas as prefab pieces and signatures, the style names, enemies per floor.
...
- Director ramp numbers — onset, ramp plies, sate, debt cap, action budget, rung thresholds/biases (§4.5), settled from the god-lab baseline corpus + phone feel together (Phase 1.5).
- Favor of the Gods (§4.5): what moves it, how far, and whether temperament is a difficulty axis, a build axis, or both.
- Furniture × the Gods (§4.6): v3 gave the gods their crate policy (weaken creates, breach destroys — §4.5), and the **conservation brake is built** (§4.5 item 4, A/B-measured 2026-09-01 — its `conserveAt`/`conserveFloor` numbers are feel-tuning territory); still open is the restlessness meter's weight for furniture smashes — full capture credit would let the player farm crates to keep the gods asleep.
- Pawn promotion-reachability lint policy (§6) — the lint itself is deferred pending the god lab's locked-pawn trajectory data; the policy question only revives if the data does.
...
- No hands/pockets as a core economy (upgrade path only). `[REVERSED 2026-09-25 — §4.10, THE DECK: spells are cards drawn from a visible deck; the pocket is the hand.]`
