...
- **Pawn double-step: the CAMP LINE** `[REVISED 2026-08-21 — designer correction; spikes 13+14]`: every pawn gets the two-square push regardless of starting position, but only **at or behind its side's starting line** — the front-most rank that side's pawns are dealt onto. Past the line, the leap is gone forever. (Spike 13's every-visit caveat is REPEALED — under it a pawn could double-step repeatedly from anywhere, and it had been recorded as "accepted" without that consequence ever being put to the designer, who rejected it on contact.) This is chess's own rule generalized: chess's double-step is ROW-based, which equals first-move-only there only because nothing can move a pawn backward — Earthquakes can, and where the readings diverge the row wins, because a player can see a line and cannot see a pawn's move history. Implementation: each deal registers its own variant with `doubleStepRegion` = every rank from the home edge to that side's camp line (per-pawn move history cannot exist — quake surgery reloads bare FENs). Three consequences, designer-accepted: a moved pawn knocked back behind the line regains the jump ("back to camp, take the run-up again"); in stacked pawn rows a rear pawn still behind the line can single-step then double; wall-scattered molding puts the line at the front-most pawn, widening shallower files' zones. Walls block both the jumped and landing squares; en passant works against any double-step. Verified in both libraries: `phase0/results/spike13-universal-doublestep.md`, `phase0/results/spike14-firstmove-doublestep.md`.
