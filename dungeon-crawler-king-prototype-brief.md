...
The second terrain glyph: **`^` — furniture** (crates, weak masonry, force fields; per-stage flavor). Neutral, immobile, owned by neither side, and **either side may capture it** by moving a piece onto it — an ordinary capture the engine generates, prices (SEE 0), and plays for and against at full strength. That engine-visibility is the point, and the price: stock FSF has no such object, so the vendored WASM pair carries the **dead-squares patch** (the design from FSF issue #609; reference implementation KOTH-Stockfish PR #29; feasibility proven end-to-end — both artifacts built from one patch, ffish↔engine perft agreement on `^` boards, `^`-free boards move-for-move identical to the stock pair; evidence and recipe in `engine/README.md`).

**Terrain everywhere except the capture itself `[designer-final]`.** To molding, slot-eating (§4.2), the camp line (§4.4), crop and the promotion-rank rule (§4.2), and the Director, a `^` is a wall. Play is the one difference, and its consequences are verified in the patched binaries:
...
- **Phase 1.2.3 — The Forge `[NEW — furniture (§4.6), part 1: the engine pair]`.** Rebuild the vendored WASM pair from patched source — the one deliberate exception to "never patch the engine," governed by CLAUDE.md rules 15–17 (the matched-pair build, the equivalence gate, the patch bar). Scope: port the dead-squares patch onto current FSF master with the reference diff's baggage stripped (`selfCapture`, `ironPieceTypes`, `deathOnCaptureTypes`, `fatalgiveaway`) and its two known movegen defects fixed (dead-captures also emitted in the QUIET stage — a performance bug, not correctness; evasion over-generation); drop its provable no-ops. Reference diff, resolved conflicts, build gotchas, and the sandbox proof: `engine/README.md`. Gate before vendoring (rule 16): `^`-free perft equivalence vs the shipped 1.1.11/0.7.9 pair for BOTH binaries (the sandbox pass proved the engine side; ffish's is still owed), ffish↔engine agreement on `^` boards, `play/selftest.html` + the 60-variant catalog in a real browser (the sandbox ran Node only — SharedArrayBuffer/pthreads untested), and a re-measure of rule 11's depth cap. File the upstream PRs from here, wait on nothing: dead squares against FSF issue #609, and the walled-passer eval fix — found during the spike, deliberately NOT in our patch (it would break eval-equivalence with the shipped pair for no play value; it reaches us only via upstream). No stage, JS, or rules change beyond the substrate.
- **Phase 1.2.4 — Set Dressing `[NEW — furniture (§4.6), part 2: in hand]`.** The game learns the glyph; the designer gets it on the phone. Retire the hard-coded `'*'` equality tests for a terrain helper (~50–60 sites across ~24 files — known landmines: `director.mjs`'s piece census reads `^` as a white piece via the `toUpperCase()` idiom, and `pushReaches` reads it as open, inverting the locked-pawn metric); give the Director the interim rule (§4.6: furniture is stone to the gods — no crate-native god behavior before the rework); a board sprite; `^` admitted to the stage map + verifier (wall-equivalent, plus the sealed-chamber warning); author furniture into all 33 stages. Exit: `^` in live duels with Earthquakes on, 10 s/move, on a phone.
- **Phase 1.2.5 — The Proving Grounds `[NEW — the calibration test bed; partly shipped]`.** Everything the §7 sweeps need before their numbers can be trusted, split out of 1.3 because the first meter-lab pass proved the old bed was not representative: four hand-authored arenas (3–5 files, 0–2 walls) with baked armies, so every measurement inherited a wrong distribution. The whole phase is **the automated-playtest rig and the data it runs on** — no duel rules change here. **✅ Shipped (PRs #10, #11):** the data bed — 33 designer-locked stages (`play/stages/`) × the army generator (`play/js/armygen.mjs`: unit bags W 3–8, molding v2.1, independent of stage geometry), stage schema v2 with the flip and crop transforms, `armygen.dealMatchup()` as the ONE composed deal entry point shared by game/verifier/lab, the static verifier (`phase0/harness/verify-stages.mjs`), and the setup screen that drives it by hand (stage picker → live army preview, one master seed, initiative/flip/crop). Two rules corrections fell out of building it and are canon: the camp-line double-step (§4.4) and crop-as-boundary-redraw with promotion always on the real far rank (§4.2). **⬜ Remaining — the lab rig:** (a) a **corpus materializer** — the converter that turns 33 stages × both orientations × both terrain arms (§4.6: stone-only via `^`→`.`, furniture as authored) × generated matchups into the stage-file sets `harness/meterlab/run.mjs` consumes (§7 player-favored edge, plus a full-strength mirror arm); (b) the **mirror-canary drift metric** in `harness/meterlab/analyze.mjs` (colour-winrate drift with material equalized = a clean Director-bias signal); (c) the **meter-lab rerun** on the new bed. Designer input needed at the START of the rerun, before compute is burned: which arms run (baseline + meter v0 + which of the nine one-knob variants + the §4.6 furniture arm), seeds/matchups per stage-orientation, and whether the favored seat stays the depth-2 proxy or moves to the human-shaped MultiPV model — switching mid-corpus forks the evidence. **Phase ends when the evidence is on the table; the rule decision is 1.3's.**
