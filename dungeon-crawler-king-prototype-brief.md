...
- **Pawn double-step is UNIVERSAL and FIRST-MOVE-ONLY** `[REVISED 2026-08-21 — designer correction; spikes 13+14]`: molding (§4.2) puts pawns at arbitrary depths, so every pawn gets the two-square push regardless of starting position — but ONLY on its first move; a pawn can never save the jump for later. (Spike 13's every-visit caveat is REPEALED: it had been recorded as "accepted" without the consequence — a pawn banking its double-step — ever being put to the designer, who rejected it on contact.) Implementation: the engine has no per-pawn move history (and quake surgery reloads bare FENs, so none could survive), but pawns never move backward, so unmoved ⇔ standing on the dealt square; each deal registers its own variant with `doubleStepRegion` = the exact squares that color's pawns are dealt onto. Known residuals, accepted as engine grammar (spike 14): a pawn arriving on a COMRADE'S dealt square (stacked-file molding only) regains the option once, and a quake displacing a pawn onto/off a dealt square shifts it. Walls block both the jumped and landing squares; en passant works against any double-step. Verified in both libraries: `phase0/results/spike13-universal-doublestep.md`, `phase0/results/spike14-firstmove-doublestep.md`.
